defmodule Ecrits.StoreTest do
  use Ecrits.DataCase, async: true

  alias Ecrits.Change
  alias Ecrits.IO.R2Stub
  alias Ecrits.Lease
  alias Ecrits.Operation
  alias Ecrits.Runtime
  alias Ecrits.Snapshot
  alias Ecrits.Store
  ...
  setup do
    ...
    original = Application.get_env(:ecrits, :io_drivers, [])
    Application.put_env(:ecrits, :io_drivers, Keyword.put(original, :r2, R2Stub))
    on_exit(fn -> Application.put_env(:ecrits, :io_drivers, original) end)
    :ok
  end
  ...
  describe "append/3" do
    ...
    test "broadcasts {:change_committed, _} on the document topic" do
      doc = new_document_id()
      lease = acquire_lease(doc)
      :ok = Phoenix.PubSub.subscribe(Ecrits.PubSub, Store.pubsub_topic(doc))
  ...
  describe "load/1" do
    ...
    test "ignores rhwp native visual snapshot rows when hydrating the runtime projection" do
      ...
      {:ok, _} =
        %Ecrits.RhwpSnapshot.Record{}
        |> Ecrits.RhwpSnapshot.Record.changeset(%{
          document_id: doc,
          revision: 1,
  ...
  describe "snapshot/2" do
    test "writes a snapshot row + R2 object at the current revision" do
      ...
      assert %Snapshot{revision: 2, r2_key: key} =
               Ecrits.LegacyRepo.get_by(Snapshot, document_id: doc, revision: 2)
    ...
    test "rolls back the DB row when R2 put fails" do
      ...
      assert nil == Ecrits.LegacyRepo.get_by(Snapshot, document_id: doc, revision: 1)
    end
  ...
  describe "document set_attr propagation (Task #81)" do
    ...
    defp setup_document_row(owner_id \\ nil, opts \\ []) do
      ...
      {:ok, _doc} =
        %Ecrits.Documents.Document{id: doc_id}
        |> Ecrits.Documents.Document.changeset(%{
          "owner_id" => owner_id,
          "title" => "Initial",
          "type_key" => type_key,
          "status" => "draft"
    ...
    test "type_key set_attr propagates for an untyped document" do
      ...
      row = Ecrits.LegacyRepo.get(Ecrits.Documents.Document, doc_id)
    ...
    test "Documents.get/2 reflects the propagated title" do
      ...
      scope = %Ecrits.Context{
        user: %Ecrits.Accounts.User{id: owner_id, email: "owner@x"},
        tenant: nil
      }
