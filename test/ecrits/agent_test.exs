defmodule Ecrits.AgentTest do
  ...
  describe "build_context/2" do
    test "produces system prompt + tools + input array" do
      ...
      assert ctx.system =~ "ecrits의 법률 문서 에이전트"
    ...
    test "loads persisted ChatThread messages before the current user turn" do
      ...
      ctx = Ecrits.Context.for_user(%Ecrits.Accounts.User{id: owner_id})
    ...
    test "instructions no longer expose hosted document tool surface" do
      ...
      refute frame.system =~ "ecrits-doc"
  ...
  describe "system_prompt/0" do
    test "is a string with the free-form agent and citation clauses" do
      ...
      assert prompt =~ "ecrits의 법률 문서 에이전트"
  ...
  describe "stale runtime guards" do
    test "RunServer cannot be started directly as a GenServer" do
      ...
      run = %Ecrits.Agent.Run{id: Ecto.UUID.generate()}
      ...
      assert {:error, {:stale_runtime_entrypoint, Ecrits.Agent.RunServer, Ecrits.Agent.Document}} =
               GenServer.start_link(Ecrits.Agent.RunServer,
                 run_id: run.id,
                 run: run,
                 action: action,
                 ctx: nil
