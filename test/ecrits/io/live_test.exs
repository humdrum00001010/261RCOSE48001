defmodule Ecrits.IO.LiveTest do
  @moduledoc """
  ...
  *skipped* with a clear reason instead of firing real API requests. To
  ...
  test "OpenAI Responses stream emits SSE events" do
    ...
    original_openai = Application.get_env(:ecrits, :openai)
    original_law = Application.get_env(:ecrits, :law_mcp)

    Application.put_env(:ecrits, :openai,
      api_key: api_key,
    ...
    Application.put_env(:ecrits, :law_mcp,
      endpoint: "https://korean-law-mcp.fly.dev/mcp",
      oc: System.get_env("LAW_OC", "openapi")
    )

    on_exit(fn ->
      Application.put_env(:ecrits, :openai, original_openai)
      Application.put_env(:ecrits, :law_mcp, original_law)
    end)
    ...
    assert {:ok, %{stream: stream, task_pid: _}} = Ecrits.IO.OpenAI.stream_chat(params)
  ...
  test "Korean Law MCP verify_citations confirms 민법 제390조" do
    original = Application.get_env(:ecrits, :law_mcp)
    ...
    on_exit(fn -> Application.put_env(:ecrits, :law_mcp, original) end)

    assert {:ok, results} = Ecrits.IO.LawMCP.verify_citations(["민법 제390조"])
