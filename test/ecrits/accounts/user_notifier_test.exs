defmodule Ecrits.Accounts.UserNotifierTest do
  @moduledoc """
  Regression test for the localhost-in-emails bug.

  `Ecrits.Accounts.UserNotifier` builds emails whose body contains an
  absolute URL produced via `EcritsWeb.url(~p"/users/log-in/<token>")`.
  That helper reads `EcritsWeb.Endpoint`'s `:url` config — so if `:url`
  ...
  body contains `https://ecrits-studio-v7zk.sprites.app/...` and NOT
  `localhost`.
  """
  use Ecrits.DataCase, async: false
  use Oban.Testing, repo: Ecrits.LegacyRepo

  alias Ecrits.Accounts
  alias Ecrits.AccountsFixtures
  alias EcritsWeb.Endpoint

  @public_host "ecrits-studio-v7zk.sprites.app"

  setup do
    original = Application.get_env(:ecrits, Endpoint) || []

    updated =
      Keyword.put(original, :url, host: @public_host, port: 443, scheme: "https")

    Application.put_env(:ecrits, Endpoint, updated)
    ...
    on_exit(fn ->
      Application.put_env(:ecrits, Endpoint, original)
  ...
  test "login-instructions email contains the public sprite URL, not localhost" do
    ...
    # Mimic the call shape from EcritsWeb.UserLive.Login / Registration:
    ...
    assert_enqueued(
      worker: Ecrits.Workers.MailerJob,
    ...
    assert email.subject == "계정 확인 안내 · ecrits"
