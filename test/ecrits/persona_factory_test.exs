defmodule Ecrits.PersonaFactoryTest do
  @moduledoc """
  Non-browser checks for `Ecrits.PersonaFactory`. These run on the default
  `mix test` invocation — the Wallaby `:browser` smoke is in
  `test/ecrits_web/features/login_persona_test.exs`.
  """
  ...
  test "build/1 produces a confirmed user, scope, and password for every persona" do
    for persona <- PersonaFactory.personas() do
      ...
      assert is_struct(user, Ecrits.Accounts.User)
      ...
      assert is_struct(scope, Ecrits.Context)
