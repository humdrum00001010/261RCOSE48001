defmodule Ecrits.StudioTest do
  ...
  setup do
    ...
    original_drivers = Application.get_env(:ecrits, :io_drivers, [])
    ...
    on_exit(fn -> Application.put_env(:ecrits, :io_drivers, original_drivers) end)
    :ok
  end

  defp scope do
    user = %Ecrits.Accounts.User{id: Ecto.UUID.generate(), email: "studio@example.com"}
  ...
  describe "open/2" do
    test "loads no-document state when no document_id is supplied" do
      ...
      assert projection == Ecrits.Runtime.State.empty_projection()
    end
  ...
  describe "command/2" do
    ...
    test "accepts a chat-only Command with document_id=nil (SPEC §4.4)" do
      ...
      assert {:ok, %Ecrits.Agent.Run{} = run} = Studio.command(s, command)
      assert is_binary(run.id)

      _ = Ecrits.Agent.Document.cancel(s, run.id)
    end
  ...
  describe "subscribe/2" do
    test "subscribes the caller to the document topic for binary document_id" do
      ...
      topic = Ecrits.Store.pubsub_topic(doc.id)
      doc_id = doc.id
      Phoenix.PubSub.broadcast(Ecrits.PubSub, topic, {:probe, doc_id})
    ...
    test "subscription lifecycle is tied to caller pid — exit cleans it up" do
      ...
      subscribers = Registry.lookup(Ecrits.PubSub, topic)
  ...
  describe "subscribe_agent/2" do
    test "subscribes to agent topic when agent_run_id is binary" do
      ...
      Phoenix.PubSub.broadcast(Ecrits.PubSub, "agent:" <> run_id, {:agent_probe, run_id})
