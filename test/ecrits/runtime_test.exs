defmodule Ecrits.RuntimeTest do
  ...
  describe "subscribe/2" do
    test "subscribes the caller to the document topic" do
      ...
      Phoenix.PubSub.broadcast(Ecrits.PubSub, Store.pubsub_topic(doc), :ping)
  ...
  describe "apply/2 → :open_document" do
    test "loads state for an existing doc; errors when document_id is missing" do
      ...
      assert {:ok, %Ecrits.Runtime.State{document_id: ^doc}} =
  ...
  describe "apply/2 → session-routed kinds" do
    ...
    test "set_contract_type is allowed once and rejected after the document is typed" do
      ...
      assert %Document{type_key: "service_agreement_v1"} =
               Ecrits.LegacyRepo.get!(Document, doc_id)
  ...
  describe "apply/2 → :chat_message" do
    test "persists the user message and routes through Ecrits.Agent.Document" do
      parent = self()
      ...
      assert {:ok, %Ecrits.Agent.Run{} = run} = Runtime.apply(@ctx, action)
