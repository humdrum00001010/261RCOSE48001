defmodule Ecrits.Export.HWPXExternalValidatorTest do
  @moduledoc """
  External-oracle validation of `Ecrits.Export.HWPX` output.
  ...
  `mix test` run. Enable via:
      ...
      mix test --include external_hwpx test/ecrits/export/hwpx_external_validator_test.exs

  The validator binary is configured at `config :ecrits, :hwpx_validator`
  ...
  setup do
    cmd = Application.fetch_env!(:ecrits, :hwpx_validator)
