defmodule Ecrits.Export.HWPXTest do
  ...
  test "Renderer.render/3 dispatches :hwpx and rejects unknown formats" do
    ...
    assert {:ok, bin, "application/hwp+zip"} = Ecrits.Export.Renderer.render(state, :hwpx, [])
    ...
    assert {:error, {:unsupported_format, :totally_made_up}} =
             Ecrits.Export.Renderer.render(state, :totally_made_up, [])
  end
