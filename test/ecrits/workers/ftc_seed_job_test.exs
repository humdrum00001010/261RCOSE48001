defmodule Ecrits.Workers.FtcSeedJobTest do
  ...
  setup do
    ...
    on_exit(fn ->
      Application.put_env(:ecrits, :io_drivers, original_drivers)
    end)
  ...
  describe "perform/1 — happy path" do
    test "creates the system user + template Document + Change", %{
      ...
      # System user landed.
      assert %Ecrits.Accounts.User{email: "system@ecrits.local"} =
               Repo.get(Ecrits.Accounts.User, FtcSeedJob.system_user_id())
  ...
  describe "Mix.Tasks.Ecrits.Seed.Ftc enqueue path" do
    test "manually enqueueing a job via Oban.insert is testable" do
      ...
      assert job.worker == "Ecrits.Workers.FtcSeedJob"
