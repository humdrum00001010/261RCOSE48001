defmodule EcritsWeb.Components.AppShellTest do
  ...
  alias EcritsWeb.Components.AppShell
  ...
  describe "app_shell/1" do
    ...
    test "renders the account menu when current_scope is signed in" do
      ...
      # landing page. The brand <a> carries the ecrits aria-label so we
      # use that as the structural anchor instead of a since-migrated
      # `.brand` class.
      assert html =~ ~r/<a[^>]*href="\/storage"[^>]*aria-label="ecrits"/u
    end
    ...
    test "anonymous brand link points to / (landing page)" do
      ...
      assert html =~ ~r/<a[^>]*href="\/"[^>]*aria-label="ecrits"/u
    end
