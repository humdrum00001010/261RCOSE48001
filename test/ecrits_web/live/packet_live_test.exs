defmodule EcritsWeb.PacketLiveTest do
  ...
  describe "packet detail" do
    ...
    test "renders attached documents and compact document actions", %{conn: conn, scope: scope} do
      ...
      assert has_element?(
               lv,
               "#packet-title-form[phx-hook='EcritsWeb.PacketLive.BlurPacketTitleOnSubmit']"
             )
    ...
    test "does not open another user's packet", %{conn: conn} do
      other_user = Ecrits.AccountsFixtures.user_fixture()
      other_scope = Ecrits.Context.for_user(other_user)
