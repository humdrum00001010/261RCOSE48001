defmodule EcritsWeb.DocumentLiveTest do
  use EcritsWeb.ConnCase, async: false

  @moduletag :legacy_saas

  import Phoenix.LiveViewTest
  import Ecrits.AccountsFixtures
  import Ecto.Query
  import Mox

  alias Ecrits.ChatThread
  alias Ecrits.Command
  alias Ecrits.LegacyRepo, as: Repo
  alias Ecrits.RhwpSnapshot.Record, as: RhwpSnapshotRecord
  alias Ecrits.Studio.State
  alias EcritsWeb.DocumentLive
  ...
  describe "mount when authenticated" do
    ...
    test "mounts at /documents/:document_id with current_document_id assigned", %{
      ...
      {:ok, doc} = Ecrits.Documents.create(scope, %{title: "Mounted"})
    ...
    test "linked document topbar points back to containing packet",
         %{conn: conn, user: user} do
      scope = Ecrits.Context.for_user(user)
      {:ok, packet} = Ecrits.Packets.create_packet(scope, %{"title" => "링크 패킷"})
      {:ok, doc} = Ecrits.Documents.create(scope, %{title: "Packet linked doc"})
      {:ok, _packet_document} = Ecrits.Packets.attach_document(scope, packet.id, doc.id)
      ...
      assert has_element?(
               lv,
               ~s(header ul[aria-label="ecrits"] a[href="/packets/#{packet.id}"]),
               "문서들"
             )

      assert has_element?(
               lv,
               ~s(header ul[aria-label="ecrits"] a.font-semibold[href="/packets/#{packet.id}"]),
               "문서들"
             )

      refute has_element?(
               lv,
               ~s(header ul[aria-label="ecrits"] a[href="/storage"]),
               "문서들"
             )
    end
    ...
    test "selected untyped document shows canvas type picker",
         %{conn: conn, user: user} do
      scope = Ecrits.Context.for_user(user)
      {:ok, doc} = Ecrits.Documents.create(scope, %{title: "Untyped draft", type_key: nil})
      ...
      {:ok, specs} = Ecrits.ContractTypes.list()
      ...
      for spec <- visible_specs do
        assert has_element?(
                 ...
                 Ecrits.ContractTypes.display_name(spec)
               )
      end
      ...
      assert Repo.get!(Ecrits.Documents.Document, doc.id).type_key == first_spec.key
    end
    ...
    test "snapshot upload refreshes the current snapshot assigns and hook attributes", %{
      ...
      old_drivers = Application.get_env(:ecrits, :io_drivers, [])

      Application.put_env(
        :ecrits,
        :io_drivers,
        Keyword.put(old_drivers, :r2, Ecrits.IO.R2Stub)
      )

      Ecrits.IO.R2Stub.reset()

      on_exit(fn ->
        Application.put_env(:ecrits, :io_drivers, old_drivers)
        Ecrits.IO.R2Stub.reset()
      end)
      ...
      assert {:ok, 2} = Ecrits.Store.latest_revision(document_id)
    ...
    test "mount replays doc_write text events newer than the rhwp snapshot", %{
      ...
      assert {:ok, %Ecrits.Change{command_kind: "doc_write", result_revision: 3}} =
               Ecrits.Runtime.apply(scope, command)
    ...
    test "empty /studio canvas offers type choices that start a typed document",
      ...
      [doc] =
        user
        |> Ecrits.Context.for_user()
        |> Ecrits.Documents.list_recent_for_scope(1)
    ...
    test "empty /studio with packet_id keeps picker and attaches typed document to packet",
         %{conn: conn, user: user} do
      scope = Ecrits.Context.for_user(user)
      {:ok, packet} = Ecrits.Packets.create_packet(scope, %{"title" => "패킷 문서"})
      ...
      assert has_element?(lv, ~s([data-role="canvas-empty-type-picker"]))
      assert Ecrits.Documents.list_recent_for_scope(scope, 1) == []
      ...
      [doc] = Ecrits.Documents.list_recent_for_scope(scope, 1)
      assert doc.type_key == first_spec.key

      {:ok, loaded_packet} = Ecrits.Packets.get_packet(scope, packet.id)
      assert Enum.any?(loaded_packet.documents, &(&1.id == doc.id))

      packet_document =
        Repo.get_by!(Ecrits.Packets.PacketDocument,
          packet_id: packet.id,
          document_id: doc.id
        )
    ...
    test "direct upload completion commits uploaded HWPX and attaches it to packet",
      ...
      scope = Ecrits.Context.for_user(user)
      {:ok, packet} = Ecrits.Packets.create_packet(scope, %{"title" => "업로드 패킷"})
      body = File.read!("test/fixtures/hwpx/real_contract.hwpx")
      object_key = "uploads/#{user.id}/direct/test.hwpx"
      {:ok, _} = Ecrits.IO.R2Stub.put(object_key, body)
      ...
      assert %Ecrits.RhwpSnapshot.Record{revision: 1, format: "hwpx"} =
               Ecrits.RhwpSnapshot.latest_for_document(doc.id, "hwpx")

      snapshot_key = "documents/#{doc.id}/snapshots/1.hwpx"
      ir_key = "documents/#{doc.id}/snapshots/1.ir.json"
      objects = Ecrits.IO.R2Stub.objects()
    ...
    test "direct upload completion opens uploaded HWPX in the RHWP editor",
      ...
      refute has_element?(opened, ~s([data-role="standard-hwp-editor"][data-matching-book]))
      refute has_element?(opened, ~s([data-role="standard-hwp-editor"][data-field-values]))

      refute has_element?(opened, ~s([data-role="custom-document-canvas"]))
    end

    test "nda_v1 standard template does not expose browser matching attrs",
         %{conn: conn, user: user} do
      scope = Ecrits.Context.for_user(user)
      ...
      assert has_element?(
               lv,
               ~s([data-role="standard-hwp-editor"][data-document-path="/assets/standard_contracts/nda_v1.hwp"])
             )
      ...
      refute has_element?(lv, ~s([data-role="standard-hwp-editor"][data-matching-book]))
      refute has_element?(lv, ~s([data-role="standard-hwp-editor"][data-field-values]))
    end

    test "standard templates no longer expose editable spec candidates",
         %{conn: conn, user: user} do
      scope = Ecrits.Context.for_user(user)
  ...
  describe "auto-grill seed on cold document open" do
    ...
    test "mount on a fresh document with body content dispatches the grill seed",
      ...
      ctx = Ecrits.Context.for_user(user)
      state = %State{selected_document_id: doc.id, mode: :editing}
      visible = Ecrits.ChatThreads.list_visible_messages(ctx, state)
    ...
    test "mount on an empty document (no projection nodes) does NOT dispatch",
      ...
      {:ok, doc} =
        Ecrits.Documents.create(scope, %{"title" => "empty-doc", "type_key" => "nda_v1"})
  ...
  describe "handle_info protocol (driven through the live LV process)" do
    ...
    test "cancel_agent cancels the Agent.Document attempt and removes transient rows",
         %{conn: conn, user: user} do
      scope = Ecrits.Context.for_user(user)
      {:ok, doc} = Ecrits.Documents.create(scope, %{"title" => "중지할 문서"})
      stub_blocking_agent_stream(self())
      on_exit(fn -> Ecrits.Agent.Document.suspend(scope, doc.id) end)
      ...
      assert assigns(lv).studio_state.agent_run_id == nil
      assert {:ok, status} = Ecrits.Agent.Document.status(scope, doc.id)
    ...
    test "chat.submit subscribes the LiveView to the new agent run PubSub topic",
         %{conn: conn, user: user} do
      scope = Ecrits.Context.for_user(user)
      {:ok, doc} = Ecrits.Documents.create(scope, %{"title" => "구독할 문서"})
    ...
    test "reasoning delta broadcast reaches the selected details body and visible thinking line",
         %{conn: conn, user: user} do
      scope = Ecrits.Context.for_user(user)
      {:ok, doc} = Ecrits.Documents.create(scope, %{"title" => "생각 표시 문서"})
      stub_blocking_agent_stream(self())
      on_exit(fn -> Ecrits.Agent.Document.suspend(scope, doc.id) end)
    ...
    test "submitting a replacement chat suspends the current Document attempt before starting the next",
         %{conn: conn, user: user} do
      scope = Ecrits.Context.for_user(user)
      {:ok, doc} = Ecrits.Documents.create(scope, %{"title" => "교체할 문서"})
      stub_blocking_agent_stream(self())
      on_exit(fn -> Ecrits.Agent.Document.suspend(scope, doc.id) end)
    ...
    test "chat rail keeps Agent.Document status internal to composer controls",
         %{conn: conn, user: user} do
      scope = Ecrits.Context.for_user(user)
      {:ok, doc} = Ecrits.Documents.create(scope, %{"title" => "상태를 표시할 문서"})
      stub_blocking_agent_stream(self())
      on_exit(fn -> Ecrits.Agent.Document.suspend(scope, doc.id) end)
  ...
  describe "command_palette_picked → set_contract_type opens the type-picker" do
    ...
    test "with a type_key, does not replace an already typed document",
      ...
      assert {:ok, %Ecrits.Change{document_id: ^document_id}} =
               Ecrits.Runtime.apply(scope, %Command{
      ...
      assert assigns(lv).projection.type_key == "employment_v1"
      assert Repo.get!(Ecrits.Documents.Document, document_id).type_key == "employment_v1"
    end
    ...
    test "typed document header renders an immutable type badge without replacement rows",
      ...
      assert has_element?(
               ...
               Ecrits.ContractTypes.display_name("employment_v1")
             )
    ...
    test "set_contract_type event leaves an already typed document unchanged",
      ...
      assert html =~ Ecrits.ContractTypes.display_name("employment_v1")
      assert assigns(lv).projection.type_key == "employment_v1"
      assert Repo.get!(Ecrits.Documents.Document, document_id).type_key == "employment_v1"
  ...
  describe "Document-pivot — Studio breadcrumb trail is 2-level" do
    ...
    test "mounting a document route gives Storage > Document breadcrumbs", %{
      ...
      scope = Ecrits.Context.for_user(user)
      {:ok, doc} = Ecrits.Documents.create(scope, %{title: "Breadcrumb draft"})
      {:ok, lv, _html} = live(conn, ~p"/documents/#{doc.id}")
  ...
  defp base_assigns(_ctx) do
    user = %Ecrits.Accounts.User{id: Ecto.UUID.generate(), email: "x@example.com"}
    scope = Ecrits.Context.for_user(user)
  ...
  defp seed_document_with_paragraph(user, content) do
    ...
    {:ok, %Ecrits.Change{}} = Ecrits.Runtime.apply(scope, command)
    doc
  end
