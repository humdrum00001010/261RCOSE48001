defmodule EcritsWeb.Live.Studio.Components.Canvas.EmptyTest do
  @moduledoc """
  ...
  alias EcritsWeb.Live.Studio.Components.Canvas.Empty

  # ---------------------------------------------------------------------------
  # Persona-perm fixtures (mirror Ecrits.PersonaFactory)
  # ---------------------------------------------------------------------------
  ...
  # are pinned in `EcritsWeb.DocumentLiveTest`); here we lock the
  ...
  describe "click events route through DocumentLive.handle_event/3" do
    ...
    test "blank button fires agent_option_picked key=blank → mints doc + navigates",
      ...
      assert Ecrits.Documents.list_recent_for_scope(scope, 5) == []
      ...
      [doc] = Ecrits.Documents.list_recent_for_scope(scope, 5)
