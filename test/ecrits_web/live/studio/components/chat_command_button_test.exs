defmodule EcritsWeb.Live.Studio.Components.ChatCommandButtonTest do
  @moduledoc """
  ...
  alias EcritsWeb.Live.Studio.Components.ChatCommandButton
  ...
  describe "Korean copy" do
    ...
    test "empty-commands state shows Korean message", %{user: _user} do
      ...
      source =
        File.read!("lib/ecrits_web/live/studio/components/chat_command_button.ex")
  ...
  describe "open via tap (LiveComponent runtime)" do
    ...
    test "tapping the trigger flips sheet_open?", %{conn: conn, user: user} do
      ...
      {:ok, lv, html} =
        live_isolated(conn, EcritsWeb.Live.Studio.Components.ChatCommandButtonTest.Host,
          session: %{"scope" => scope}
        )
