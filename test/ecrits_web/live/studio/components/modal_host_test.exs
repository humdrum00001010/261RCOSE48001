defmodule EcritsWeb.Live.Studio.Components.ModalHostTest do
  @moduledoc """
  ...
  alias EcritsWeb.Live.Studio.Components.ModalHost
  ...
  describe "render_component/2 — one flag per modal" do
    ...
    test "type-picker modal renders rows headlined by the localized name in :ko locale" do
      previous = Gettext.get_locale(EcritsWeb.Gettext)
      Gettext.put_locale(EcritsWeb.Gettext, "ko")
      on_exit(fn -> Gettext.put_locale(EcritsWeb.Gettext, previous) end)
      ...
      {:ok, nda} = Ecrits.ContractTypes.get(nil, "nda_v1")
  ...
  describe "close-modal interactions via the parent LV" do
    ...
    test "Esc colocated hook is mounted on the modal host shell" do
      ...
      assert html =~ "phx-hook=\"EcritsWeb.Live.Studio.Components.ModalHost.ModalEsc\""
    end
  end
