defmodule Mix.Tasks.Ecrits.ImagegenTest do
  @moduledoc """
  Smoke for `mix ecrits.imagegen`. Asserts:
  ...
  describe "task runner" do
    setup do
      ...
      Application.put_env(:ecrits, :imagegen_req_options, plug: {Req.Test, :imagegen})

      System.put_env("OPENAI_API_KEY", "test-imagegen-key")

      on_exit(fn ->
        Application.delete_env(:ecrits, :imagegen_req_options)
        File.rm_rf!(tmp_root)
      end)
    ...
    test "is callable and writes a PNG via the stubbed plug", ctx do
      assert Code.ensure_loaded?(Mix.Tasks.Ecrits.Imagegen)
      assert function_exported?(Mix.Tasks.Ecrits.Imagegen, :run, 1)

      refute File.exists?(ctx.tmp_output)

      assert :ok =
               Mix.Tasks.Ecrits.Imagegen.run([
                 "--manifest",
                 ctx.tmp_manifest,
                 "--force"
               ])
    ...
    test "is idempotent — skips when output already exists", ctx do
      ...
      assert :ok = Mix.Tasks.Ecrits.Imagegen.run(["--manifest", ctx.tmp_manifest])
  ...
  describe "missing api key" do
    ...
    test "raises Mix.Error loudly when OPENAI_API_KEY is unset" do
      ...
      assert_raise Mix.Error, ~r/OPENAI_API_KEY/, fn ->
        Mix.Tasks.Ecrits.Imagegen.run(["--force"])
      end
    end
  end
end
