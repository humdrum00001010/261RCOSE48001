defmodule EcritsWeb.AuthComponents do
  @moduledoc """
  ...
  import EcritsWeb.CoreComponents, only: [icon: 1]
      ...
      <aside class="hidden lg:flex flex-col justify-between p-10 xl:p-14 bg-base-200/40 border-r border-base-200">
        <div class="space-y-2">
          ...
          <p class="text-xs text-base-content/50 tracking-wide uppercase">
            {dgettext("auth", "Ecrits for small-business operators")}
          </p>
        </div>
        ...
        <div class="space-y-2 text-xs text-base-content/50">
          ...
          <p>
            {dgettext("auth", "© %{year} Ecrits", year: DateTime.utc_now().year)}
          </p>
        </div>
      </aside>
