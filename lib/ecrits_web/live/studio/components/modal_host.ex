defmodule EcritsWeb.Live.Studio.Components.ModalHost do
  @moduledoc """
  Studio modal host (Wave 3C1 / modal-host).

  Single `Phoenix.LiveComponent` that owns every modal-style overlay in
  Studio. The parent `EcritsWeb.DocumentLive` toggles flags on
  ...
  # `Ecrits.ContractTypes.list/0` so it stays in sync with the registry.
