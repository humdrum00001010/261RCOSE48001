defmodule EcritsWeb.DocumentLive do
  @moduledoc """
  ...
  follow-up.
      ...
      socket.assigns = %{
        ...
        # Set by mount
        studio_state: %Ecrits.Studio.State{
  ...
  alias Ecrits.ChatThreads
  alias Ecrits.Command
  alias Ecrits.ContractTypes
  alias Ecrits.Documents
  alias Ecrits.Packets
  alias Ecrits.Studio
  alias EcritsWeb.Components.Breadcrumbs
  alias EcritsWeb.Components.CommandPalette
  alias EcritsWeb.Live.Studio.Components
    ...
    case Studio.open(scope, params) do
      ...
      {:error, reason} ->
        socket =
          ...
          |> assign(:studio_state, %Ecrits.Studio.State{mode: :no_document})
  ...
  def handle_event("rhwp.text.mutated", envelope, socket) do
    ...
    case Ecrits.Runtime.apply(scope, command) do
      {:ok, _change} ->
        :ok
  ...
  def handle_event("rhwp.snapshot.upload", %{"bytes_base64" => encoded} = params, socket)
    ...
    with true <- is_binary(document_id),
         ...
         {:ok, format} <- Ecrits.RhwpSnapshot.normalize_format(to_string(format || "")),
         {:ok, revision} <- Ecrits.Store.latest_revision(document_id),
         :ok <- verify_rhwp_min_revision(revision, min_revision),
         {:ok, bytes} <- Base.decode64(encoded),
         {:ok, snapshot} <-
           Ecrits.RhwpSnapshot.upload_and_commit(document_id, revision, bytes, ir, format) do
  ...
  defp create_blank_document(socket, type_key \\ nil) do
    ...
    case Ecrits.Runtime.apply(scope, action) do
      {:ok, %Ecrits.Change{document_id: document_id}} ->
  ...
  defp persist_direct_uploaded_document(socket, action, body, params, object_key, format) do
    ...
    case Ecrits.Runtime.apply(scope, action) do
      {:ok, %Ecrits.Change{document_id: document_id, result_revision: revision}} ->
        persist_result =
          with {:ok, _snapshot} <-
  ...
  defp commit_direct_upload_snapshot(document_id, revision, body, params, object_key, format) do
    ...
    Ecrits.RhwpSnapshot.upload_and_commit(document_id, revision, body, ir, format)
  end
  ...
  defp r2_driver do
    ...
    |> Keyword.get(:r2, Ecrits.IO.RetiredObjectStore)
  end
  ...
  def dispatch(socket, %Command{} = action) do
    ...
    case Studio.command_result(scope, state, action) do
      {:ok, %Ecrits.Studio.State{} = new_state, result} ->
        socket
  ...
  defp cancel_agent_run(scope, state, run_id) do
    case Ecrits.Agent.Document.cancel(scope, run_id) do
      {:error, :not_found} ->
        Ecrits.Agent.Document.suspend(scope, state.selected_document_id)
  ...
  defp refresh_agent_document_status(socket) do
    ...
    |> update(:studio_state, fn
      %Ecrits.Studio.State{} = state -> %{state | agent_run_id: current_run_id}
      state -> state
    end)
  end

  defp fetch_agent_document_status(scope, %Ecrits.Studio.State{} = state) do
    case Ecrits.Agent.Document.status(scope, state.selected_document_id) do
      {:ok, status} -> status
      {:error, _reason} -> nil
    end
  end
  ...
  defp apply_submit_result(socket, %Command{kind: :set_contract_type}, %Ecrits.Change{}) do
  ...
  defp reload_current_document(socket) do
    ...
    case Studio.reload(scope, state) do
      {:ok, {%Ecrits.Studio.State{} = new_state, projection}} ->
        socket
        |> assign(:studio_state, new_state)
        |> assign_projection(projection)
        |> assign_current_document_context(scope, new_state)
  ...
  @doc """
  Translates a UI event name + params into a typed `Ecrits.Command`. Returns
  ...
  def handle_protocol_message({:studio_loaded, %Ecrits.Studio.State{} = state}, socket) do
  ...
  def handle_protocol_message({:change_committed, %Ecrits.Change{} = change}, socket) do
    socket
    ...
    <%= if @viewport == :mobile do %>
      ...
      >
        ...
        <.live_component
          ...
          documents={Ecrits.Studio.list_documents(@current_scope)}
        />
  ...
  defp current_document(scope, %Ecrits.Studio.State{selected_document_id: document_id})
       when is_binary(document_id) do
  ...
  defp packet_for_current_document(scope, %Ecrits.Studio.State{
  ...
  defp app_shell_nav_label(%Ecrits.Packets.Packet{}), do: "문서들"
  defp app_shell_nav_label(_packet), do: "문서들"

  defp app_shell_nav_path(%Ecrits.Packets.Packet{id: packet_id}),
  ...
  defp assign_projection(socket, projection) do
    assign(socket, :projection, projection)
  end
  ...
  defp load_rhwp_text_events(document_id, base_revision) do
    case Ecrits.Store.changes_since(document_id, base_revision) do
      {:ok, changes} ->
        changes
        |> Enum.filter(&(&1.command_kind in @rhwp_text_command_kinds))
        |> Enum.flat_map(fn %Ecrits.Change{payload: payload, result_revision: revision} ->
          payload
          |> List.wrap()
  ...
  defp load_rhwp_snapshot_candidates(document_id, format) do
    document_id
    |> Ecrits.RhwpSnapshot.candidates_for_document(format, limit: 5)
    |> Enum.map(&snapshot_candidate(document_id, &1))
  end

  defp snapshot_candidate(document_id, %Ecrits.RhwpSnapshot.Record{
         revision: rev,
         format: format,
         projection: projection
       }) do
  ...
  defp unregister_rhwp_materializer_editor(%Ecrits.Studio.State{
         selected_document_id: document_id
       }) do
  ...
  defp call_rhwp_materializer(function, args) do
    module = Ecrits.RhwpSnapshot.Materializer
  ...
  defp push_rhwp_remote_text(socket, %Ecrits.Change{command_kind: kind} = change)
  ...
  defp append_rhwp_text_events(
         socket,
         %Ecrits.Change{command_kind: kind, result_revision: revision} = change
       )
       when kind in @rhwp_text_command_kinds do
    events =
      change.payload
  ...
  defp maybe_subscribe_test_operation_blocks(%Ecrits.Context{user: %{id: user_id}})
       when is_binary(user_id) do
    if Application.get_env(:ecrits, :test_auth, false) do
      Phoenix.PubSub.subscribe(Ecrits.PubSub, test_operation_topic(user_id))
    else
      :ok
    end
  end
  ...
  defp breadcrumb_document(scope, %{selected_document_id: document_id}, _projection)
       when is_binary(document_id) do
    case Ecrits.Documents.get(scope, document_id) do
  ...
  defp empty_projection, do: Ecrits.Runtime.State.empty_projection()
  ...
  # scoped op targets.
  defp mark_agent_authored_nodes(state, %Ecrits.Change{
         actor_type: :agent,
         payload: payload
       })
    ...
    if node_ids == [] do
      ...
      Ecrits.Studio.State.mark_recently_authored(
        state,
        node_ids,
