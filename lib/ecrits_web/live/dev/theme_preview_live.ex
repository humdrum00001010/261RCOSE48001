defmodule EcritsWeb.Dev.ThemePreviewLive do
  @moduledoc """
  `/dev/theme` — colour + typography swatch for the DaisyUI `studio` theme.

  Mounted under a `:dev_only` route gate (see `EcritsWeb.Router`) so it
  ...
  def render(assigns) do
    ...
    <div class="min-h-screen bg-base-100 text-base-content p-6 font-sans">
      <header class="flex items-center justify-between mb-6">
        <div>
          <h1 class="text-2xl font-semibold">Ecrits — theme preview</h1>
          <p class="text-sm opacity-70">
            DaisyUI <code>studio</code> / <code>studio-dark</code> tokens
          </p>
        </div>
        <EcritsWeb.Layouts.theme_toggle />
      </header>
