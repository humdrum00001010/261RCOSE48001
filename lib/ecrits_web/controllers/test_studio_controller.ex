if Application.compile_env(:ecrits, :test_auth, false) do
  defmodule EcritsWeb.TestStudioController do
    @moduledoc """
    Playwright-only Studio browser QA hooks.

    Gated behind `Application.compile_env(:ecrits, :test_auth, false)`: in
    ...
    def operation_blocks(conn, _params) do
      with {:ok, user} <- authenticated_user(conn) do
        ...
        topic = EcritsWeb.DocumentLive.test_operation_topic(user.id)

        Phoenix.PubSub.broadcast(Ecrits.PubSub, topic, {
          :tool_call_started,
          run_id,
          %{
            id: tool_id,
    ...
    defp authenticated_user(conn) do
      with token when is_binary(token) <- get_session(conn, :user_token),
           {%Ecrits.Accounts.User{} = user, _inserted_at} <-
