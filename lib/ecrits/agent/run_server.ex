defmodule Ecrits.Agent.RunServer do
  @moduledoc """
  Stale compatibility namespace for the retired per-run runtime.

  Agent runtime ownership now lives in `Ecrits.Agent.Document`, one
  ...
  def via(run_id), do: {:via, Registry, {Ecrits.Agent.Document.RunRegistry, run_id}}

  def whereis(run_id) do
    Logger.warning(
      "Ecrits.Agent.RunServer.whereis/1 is stale for #{inspect(run_id)}; " <>
        "use Ecrits.Agent.Document.whereis/1"
    )

    Ecrits.Agent.Document.whereis(run_id)
  end
  ...
  def whereis_for_scope(user_id, document_id)
    ...
    Logger.warning(
      "Ecrits.Agent.RunServer.whereis_for_scope/2 is stale for " <>
        "#{inspect({user_id, document_id})}; use Ecrits.Agent.Document.whereis_for_scope/2"
    )

    Ecrits.Agent.Document.whereis_for_scope(user_id, document_id)
  end
  ...
  defp stale_runtime_entrypoint do
    {:stale_runtime_entrypoint, __MODULE__, Ecrits.Agent.Document}
  end
end
