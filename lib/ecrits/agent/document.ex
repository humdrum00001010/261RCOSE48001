defmodule Ecrits.Agent.Document do
  @moduledoc """
  ...
  alias Ecrits.Agent
  alias Ecrits.Agent.Run
  alias Ecrits.Command
  alias Ecrits.Context
  alias Ecrits.Types, as: T

  @registry Ecrits.Agent.Document.Registry
  @run_registry Ecrits.Agent.Document.RunRegistry
  ...
  def init(args) do
    {:ok,
     %__MODULE__{
       ...
       openai: Application.fetch_env!(:ecrits, :io_drivers)[:openai]
     }}
  end
  ...
  def handle_info({:stream_failed, run_id, reason}, state) do
    with %{run: %{id: ^run_id}} = attempt <- state.current do
      ...
      _ = Ecrits.ChatThreads.append_assistant_message(attempt.ctx, attempt.action, message)
  ...
  defp ensure_runtime(key, ctx, document_id) do
    case whereis_key(key) do
      nil ->
        ...
        case Ecrits.Agent.DocumentSupervisor.start_document(args) do
          {:ok, pid} -> {:ok, pid}
  ...
  defp complete_attempt(state, attempt, final_text) do
    ...
    case decoder.(final_text, run_id: attempt.run.id, turn_index: attempt.run.turn_index) do
      {:ok, action} ->
        ...
        _ = Ecrits.ChatThreads.append_assistant_message(attempt.ctx, attempt.action, reply)
      ...
      {:error, reason} ->
        ...
        _ =
          Ecrits.ChatThreads.append_assistant_message(attempt.ctx, attempt.action, placeholder)
  ...
  defp persist_failed_tool_call(attempt, reason) do
    ...
    Ecrits.ChatThreads.append_tool_call_message(thread_id, operation)
  end
