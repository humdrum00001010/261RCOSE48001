defmodule Ecrits.AcpAgent.AcpStream do
  ...
  defp reusable_client(opts) do
    ...
    if is_pid(client) and Process.alive?(client) and is_present(session_id) do
      {:ok, client, session_id}
    else
      :none
    end
  ...
  defp open_provider_session(client, cwd, prior_session_id, timeout, mcp_servers)
       when is_present(prior_session_id) do
    if resume_supported?(client) do
      case Client.load_session(client, prior_session_id, cwd,
             timeout: timeout,
             mcp_servers: mcp_servers
           ) do
        {:ok, %{"sessionId" => session_id}} when is_present(session_id) ->
          {:ok, session_id}
  ...
  defp map_update(%{"sessionUpdate" => "agent_message_chunk"} = update, state) do
    case {update_text(update), final_message?(update), state.saw_text?} do
      {text, true, false} when is_present(text) ->
        {:event, %{type: :text_delta, delta: text}, %{state | saw_text?: true}}

      {_text, true, true} ->
        {:skip, state}

      {text, false, _saw_text?} when is_present(text) ->
        {:event, %{type: :text_delta, delta: text}, %{state | saw_text?: true}}
  ...
  # A provider can report a TERMINAL failure inside an otherwise unremarkable
  # update's `_meta` rather than as a `sessionUpdate: "error"` — codex reports
  # usage-limit exhaustion this way ("You've hit your usage limit…") on a
  # `session_info_update`. The generic clause below skips those, so the turn ends
  # `end_turn` with no text and the rail shows an EMPTY agent bubble with no
  # explanation. Surface the provider's own message instead of swallowing it.
  defp map_update(update, state) when is_map(update) do
    case adapter_meta_error(update) do
      message when is_present(message) ->
        {:error, message, state}
  ...
  defp adapter_meta_error(update) do
    case get_in(update, ["_meta", "ex_mcp", "error"]) do
  ...
  defp map_tool_call_status(update, tool_call_id, name, kind, state) do
    case Map.get(update, "status") do
      "completed" when is_present(name) ->
        {:event,
         %{
