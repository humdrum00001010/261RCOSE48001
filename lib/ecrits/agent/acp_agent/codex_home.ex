defmodule Ecrits.AcpAgent.CodexHome do
  ...
  defp write_document_playbook(home, true) do
    ...
    The open document is one `.doclang.xml` DocLang document: a single XML tree
    with exactly one `<doclang>` root. Edit the element whose text you are
    changing and leave the rest of the tree byte-identical — element order and
    nesting ARE the addressing, so inserting, deleting or reordering elements
    moves every later address. Keep `<custom>` blocks untouched: they carry the
    properties (font, colour, named style) DocLang itself cannot express, and
    dropping one silently discards that formatting.
    Text newlines stay escaped; do not reflow or re-indent the tree.
    ...
    2. Use native search/read plus `apply_patch` or Python/Ruby. Python/Ruby
       ...
       target.
    3. Read that exact element immediately before patching and build the
       replacement from the fresh read (space runs in forms are width-exact).
    4. For a small edit, patch that element in place. For a scripted batch,
       transform one fresh full read in memory, write the complete result to the
       exact sibling `.tmp` file, close it, then atomically rename it over the
       target once. Never truncate the mounted target in place, use an external
       temp, or run concurrent writers: one writer and one rename per batch.
    5. After the write returns, wait for the VFS write-back to settle. Boundedly
       retry reading until the projection parses as a complete DocLang document
       and the intended elements are present. **Every partial prefix of the file
       is a parse error, never a shorter valid document** — so a mid-write read
       failing is normal in-flight state: wait and retry, do not repair or
       rewrite transient bytes. Then discard every saved line number and prior
       match. `EINVAL` means reread canonical bytes and restage once. Never
       delete or rename a rejected projection temp file.
    ...
    A search for `계약명` hits a `<paragraph>` element. Read that element,
    replace only the target text inside it, and write the tree back with every
    other element byte-identical. That is the entire edit.

    ## Inserts

    Everything inserts through the mounted file, as XML elements — there are no
    lines, no payload arrays and no JSON objects in this format. For a requested
    table after Article 51 and before the annex, insert a `<table>` element at
    that position in the tree. A NEW PARAGRAPH is a new `<paragraph>` element at
    the right position. Never tell the user a new paragraph is impossible.

    Element order and nesting ARE the addressing, so any insert or delete shifts
    the address of every later element: re-read before addressing anything after
    the edit. Never put raw newlines inside an element's text; add a sibling
    element instead. Never invent or copy `<custom>` blocks — they carry the
    font/colour/named-style properties DocLang itself cannot express, so copying
    one onto a new element silently transplants another element's formatting.
