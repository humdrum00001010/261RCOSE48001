defmodule Ecrits.AcpAgent.Session do
  ...
  ## Single first receiver, single producer

  Two unrelated streams reach this process, and BOTH leave it as one
  `Ecrits.Agent.Event` on `agent:<session_id>`:

    * `{:acp_session_update, _sid, update}` — narration only (text/reasoning
      deltas, tool_call/tool_call_update). No ACP adapter ever emits a
      filesystem write, so this stream carries no patch bytes.

    * `{:vfs_doc_edited, info}` on `doc_vfs:<canonical workspace root>` — THE
      document patch. The agent's adapter-native Edit/Write landed on the
      exfuse mount and `Ecrits.Doc.Projection` / `Ecrits.Fuse.OpenDocs`
      broadcast it. Every session in a workspace hears the topic, so ownership
      is resolved here: an edit stamped with ANOTHER session's `agent_id` is
      dropped, while its own and unowned edits (a user's write, a `doc.save`)
      are re-emitted. The chat rail no longer subscribes to `doc_vfs:`
      directly; routing through `emit/2` is what gives a VFS edit the same
      `event_seq` / `instance_id` / ordering as every ACP event.
  ...
  the document THIS agent is bound to (`state.pool_document_id`, the
  `Ecrits.Doc.DocumentId` handle) — never a global active doc. `active_doc` is nil before any document is
  bound.
  """
  ...
  def with_turn_commit(pid, identity, fun)
    ...
    case Map.get(identity, :turn_id) do
      turn_id when is_present(turn_id) ->
  ...
  def init(opts) do
    ...
    {:ok,
     %{
       ...
       # `doc_vfs:<canonical root>` — the topic carrying THE document patch (see
       # the moduledoc). Subscribed here, never in a LiveView, so an
       # adapter-native Edit/Write on the exfuse mount becomes an
       # `Ecrits.Agent.Event` with the same cursor as every ACP event.
       doc_vfs_topic: subscribe_doc_vfs(nil, Keyword.get(opts, :workspace_root)),
       ...
       # The agent's ACTIVE doc for the doc.* MCP tools: the `Ecrits.Doc.DocumentId`
       # document id (`d_<kind>_<hash>`) of the doc this agent is bound to.
       # `doc.context` returns THIS (per-agent, never a global active doc);
       ...
       # `edit_id`s this session has already started a Layer-1.5 patch worker
       # for, most recent first. Bounded: it exists only to keep a SETTLED patch
       # from being resurrected by a trailing event (`:snapshot_ready` is
       # published after `:committed`), and an in-flight patch is recognised by
       # its registered process, not by this list.
       patch_edit_ids: []
     }}
  end
  ...
  def handle_call({:prepare_restart, workspace_root}, _from, state) do
    state =
      state
      |> put_workspace_root(workspace_root)
      |> ensure_instance_id()
      |> ensure_queue()
      |> ensure_cancellation_fence()
      |> ensure_terminal_finalization()
  ...
  # THE document patch. Every session in a workspace hears `doc_vfs:<root>`, so
  # ownership is decided here (`info.agent_id == state.id`) and the owning
  # session re-emits it as one `Ecrits.Agent.Event` on `agent:<id>`. The raw
  # lifecycle map rides in `payload` untouched — the rail's preview/timeline
  # logic already speaks it, and nothing about the patch is re-derived here.
  def handle_info({:vfs_doc_edited, info}, state) when is_map(info) do
    {:noreply, emit_vfs_doc_edit(state, :doc_edited, info)}
  end

  # A provisional preview whose atomic rename was refused. Same route, so the
  # rail can retract the card it was told to draw, in cursor order.
  def handle_info({:vfs_doc_edit_rejected, info}, state) when is_map(info) do
    {:noreply, emit_vfs_doc_edit(state, :doc_edit_rejected, info)}
  end
        ...
        state =
          case outcome do
            ...
            {:failed, reason, :close} ->
              ...
              |> close_acp_client()
              |> record_transcript_turn(current)
              |> emit(%{type: :turn_failed, turn_id: turn_id, reason: failure_reason(reason)})
  ...
  defp renotify_active_turn_owner(%{current: %{turn_id: turn_id, task_pid: task_pid}} = state)
       when is_present(turn_id) and is_pid(task_pid) do
    _ =
  ...
  defp launch_turn(
    ...
    # Advances the turn a mount registration is pinned to, so `write_back`'s
    # fence rejects a SUPERSEDED turn rather than the same agent legitimately
    # continuing. A no-op since `doc.open_doc` was deleted — registrations were
    # its only source — and it dies with `OpenDocs` in step 4 of
    # docs/plans/2026-07-28-client-owns-document.md; kept until then so the fence
    # keeps its only advancer if a registrar returns.
    Ecrits.Fuse.OpenDocs.refresh_agent_turn(state.id, state.instance_id, turn_id)
  ...
  defp handle_turn_event(
       ...
       when is_pid(client) and is_present(id) do
    state = ensure_acp_client_fields(state)
  ...
  defp different_preview_identity?(left, right)
       when is_present(left) and is_present(right),
       do: left != right
  ...
  defp mark_thread_change(state, id) do
    ...
    if is_present(prior) and prior != id and rows > 0 do
  ...
  defp persist_durable_state(%{workspace_root: workspace_root, id: id} = state)
       when is_present(workspace_root) and is_present(id) do
    _ =
  ...
  # The ONE exit from this process. Both incoming streams end here, as one
  # `Ecrits.Agent.Event` stamped with the process incarnation and the
  # process-owned cursor a re-attaching rail orders against its snapshot.
  defp emit(state, event) do
    ...
    event =
      ...
      |> struct!(
        session_id: state.id,
        instance_id: state.instance_id,
        # `:event_seq` is the name already on the wire; `:seq` is the plan's
        # name for the same cursor. One assignment, so they cannot drift.
        event_seq: event_seq,
        seq: event_seq,
        at: DateTime.utc_now() |> DateTime.truncate(:millisecond) |> DateTime.to_iso8601()
      )

    state = ensure_patch_worker(state, event)

    Phoenix.PubSub.broadcast(@pubsub, topic(state.id), {:agent_event, event})
    maybe_persist_durable_event(state, event)
  end

  # Layer 1.5 (`Ecrits.Doc.EditSession`) is started HERE, from the single
  # producer, and deliberately not from a LiveView:
  #
  #   * ORDERING. `start_patch/2` returns only after the worker's `init/1` has
  #     subscribed, so starting it before the broadcast below guarantees the
  #     worker sees the very first event of its patch. A rail can only react to
  #     an event it has already received, so a view-side start would always be
  #     one event late.
  #   * NO BROWSER REQUIRED. A turn runs whether or not a socket is attached;
  #     the patch still accumulates, and a rail that mounts mid-patch recovers
  #     it with `EditSession.snapshot/1`.
  #   * ONE START. N tabs on the same workspace would otherwise race to start
  #     (and re-seed) the same worker.
  #
  # Scoped to the AUTHORITATIVE patch stream. `:edit_delta` /
  # `:file_change_snapshot` are provider-PROPOSED display state — the plan's
  # Layer 2 is explicit that ACP narration "Carries NO patch bytes", the rail
  # already refuses to let those touch a document, and their ids are not the VFS
  # `edit_id` space. Starting a worker on one would leak a process per proposal
  # and could collide with a real patch id. Extend this list when a new stream
  # actually carries document truth.
  #
  # Cheap on the hot path: an in-flight patch is a registry (ETS) lookup, and
  # narration never reaches the body at all.
  @patch_event_types [:doc_edited, :doc_edit_rejected]

  defp ensure_patch_worker(state, %Event{type: type, edit_id: edit_id} = event)
       when type in @patch_event_types and is_present(edit_id) do
    cond do
      not owned_patch?(state, event) ->
        # An UNOWNED edit (a user's own save, a `doc.save`) is emitted so an open
        # viewer resyncs, but it belongs to no conversation and draws no card.
        # Starting a worker for it would leak a process AND make a re-mounting
        # rail recover a card the live rail refuses to draw.
        state

      is_pid(EditSession.whereis(edit_id)) ->
        state

      patch_started?(state, edit_id) ->
        # Already settled. `:snapshot_ready` is published AFTER `:committed`, so
        # without this the trailing event would resurrect a finished patch.
        state

      true ->
        EditSession.start_patch(edit_id,
          session_id: state.id,
        ...
        remember_patch(state, edit_id)
    end
  end

  defp ensure_patch_worker(state, _event), do: state

  defp owned_patch?(state, event), do: Event.field(event, :agent_id) == state.id

  # Bounded so a long conversation cannot grow this without limit. Trimming an
  # ancient `edit_id` is safe: its patch has long since stopped emitting.
  @max_tracked_patches 256

  defp patch_started?(state, edit_id), do: edit_id in Map.get(state, :patch_edit_ids, [])

  defp remember_patch(state, edit_id) do
    tracked = Map.get(state, :patch_edit_ids, [])
    Map.put(state, :patch_edit_ids, Enum.take([edit_id | tracked], @max_tracked_patches))
  end

  # Every session in a workspace is subscribed to the same `doc_vfs:` topic, so
  # ownership is decided here:
  #
  #   * `agent_id == state.id` — mine. Emit; the rail draws a patch card.
  #   * no `agent_id` — unowned (a user's own write, a `doc.save`). Emit: it
  #     belongs to no chat rail, but an OPEN viewer still has to resync. The
  #     rail's own owner check keeps it out of the transcript.
  #   * `agent_id` set to some other session — theirs. Drop, so a workspace's
  #     second rail never renders a neighbour's edit.
  defp emit_vfs_doc_edit(state, type, info) do
    if vfs_doc_edit_visible?(state, item_field(info, :agent_id)) do
      emit(state, %Event{
        type: type,
        turn_id: item_field(info, :turn_id),
        edit_id: item_field(info, :edit_id),
        document_id: item_field(info, :document_id),
        revision: item_field(info, :revision),
        base_revision: item_field(info, :base_revision),
        path: item_field(info, :path),
        phase: vfs_doc_edit_phase(type, info),
        payload: info
      })
    else
      state
  ...
  # A legacy VFS broadcaster names no `phase`. The chat rail has always read that
  # absence as "committed" (or `:candidate` for a preview-only write), so the
  # patch worker must read it the same way — otherwise a patch whose terminal
  # event is IMPLICIT never settles, and its worker lives until the idle guard
  # reaps it. `payload` keeps the raw map untouched; only the typed field is
  # derived.
  defp vfs_doc_edit_phase(:doc_edit_rejected, _info), do: :rejected

  defp vfs_doc_edit_phase(_type, info) do
    case normalize_edit_phase(item_field(info, :phase)) do
      nil -> if item_field(info, :preview_only) == true, do: :candidate, else: :committed
      phase -> phase
    end
  end

  @edit_phases [:candidate, :committed, :rejected, :snapshot_ready]

  defp normalize_edit_phase(phase) when phase in @edit_phases, do: phase

  defp normalize_edit_phase(phase) when is_binary(phase),
    do: Enum.find(@edit_phases, &(Atom.to_string(&1) == phase))

  defp normalize_edit_phase(_phase), do: nil

  defp vfs_doc_edit_visible?(state, agent_id) when is_present(agent_id),
    do: agent_id == state.id

  defp vfs_doc_edit_visible?(_state, _agent_id), do: true

  # A provider's terminal error is user-facing text, not a debugging artefact:
  # `inspect/1` on `{:acp_error, "You've hit your usage limit…"}` used to put the
  # raw tuple in the chat bubble. Unwrap the shapes `AcpStream` produces and keep
  # `inspect/1` only for genuinely unstructured reasons.
  defp failure_reason({tag, message}) when tag in [:acp_error, :exception] and is_binary(message),
    do: message

  defp failure_reason(reason) when is_binary(reason), do: reason
  defp failure_reason(reason), do: inspect(reason)

  # Re-point the doc-VFS subscription when the session's workspace changes
  # (restart / reconcile). Idempotent: the same canonical root is a no-op.
  defp put_workspace_root(state, workspace_root) do
    previous = Map.get(state, :doc_vfs_topic)

    state
    |> Map.put(:workspace_root, workspace_root)
    |> Map.put(:doc_vfs_topic, subscribe_doc_vfs(previous, workspace_root))
  end

  defp subscribe_doc_vfs(previous_topic, workspace_root) do
    topic = doc_vfs_topic(workspace_root)
    ...
    cond do
      ...
      true ->
        if is_binary(previous_topic), do: Phoenix.PubSub.unsubscribe(@pubsub, previous_topic)
        if is_binary(topic), do: Phoenix.PubSub.subscribe(@pubsub, topic)
        topic
    end
  end

  defp doc_vfs_topic(workspace_root) when is_present(workspace_root),
    do: "doc_vfs:" <> DocMount.canonical_root(workspace_root)

  defp doc_vfs_topic(_workspace_root), do: nil
  ...
  defp ensure_instance_id(state) do
    case Map.get(state, :instance_id) do
      instance_id when is_present(instance_id) -> state
