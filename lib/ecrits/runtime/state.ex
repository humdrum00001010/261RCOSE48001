defmodule Ecrits.Runtime.State do
  @moduledoc """
  The in-memory projection of a document that `Ecrits.Session.Reducer`
  compiles against and the Session hydrates from `Ecrits.Store`.
  ...
  The `:projection` map is the pure data the Engine mutates. Its shape:
      ...
      %{
        title:      String.t() | nil,
        type_key:   Ecrits.Types.contract_type_key() | nil,
        metadata:   map(),
  ...
  `Ecrits.Engine` `:set_attr` validation. Additive — extra keys are not
