defmodule Ecrits.Store do
  ...
  `append/3` is the only mutation entrypoint. It:
    ...
    2. Acquires a Postgres advisory lock keyed by the document id so
       ...
       in-database mutex.
    3. Calls `Ecrits.Lease.assert_current!/2` to verify the supplied
  ...
  `load/1` fetches the most recent `Ecrits.Snapshot` row (if any),
  ...
  def append(document_id, %Change{} = change, fencing_token) when is_integer(fencing_token) do
    ...
    case result do
      ...
      {:ok, %Change{} = persisted} ->
        ...
        _ = Ecrits.Documents.touch_revision(document_id, persisted.result_revision)
  ...
  defp do_append(document_id, %Change{} = change) do
    ...
    case Ecrits.Documents.guard_body_mutation(document_id, change) do
      :ok -> :ok
  ...
  defp apply_document_attr(doc_id, %Operation{args: args}) do
    ...
    case key do
      :title when is_binary(value) -> Ecrits.Documents.set_title(doc_id, value)
      :type_key -> Ecrits.Documents.set_type(doc_id, value)
      :status -> Ecrits.Documents.set_status(doc_id, value)
  ...
  @doc """
  Convert a persisted `Ecrits.Change` row back into a `ChangeInput` so
  ...
  `Ecrits.Operation` struct with atomized fields. Exposed for callers
  like `Ecrits.Session` that need to replay stored `inverse_ops` through
  the Engine.
  """
  ...
  # Node kinds produced by `Ecrits.IO.Upstage.map_category/1` plus the
