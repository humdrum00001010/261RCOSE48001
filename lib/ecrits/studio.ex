defmodule Ecrits.Studio do
  ...
  Studio is the thin product seam between `EcritsWeb.DocumentLive` and
  `Ecrits.Runtime`. It does **not** own document truth — `Store` is truth.
  It only:

    * shapes a `%Ecrits.Studio.State{}` for the LV to render against;
  ...
  defp do_open(ctx, document_id) do
    with {:ok, _doc} <- Ecrits.Documents.get(ctx, document_id),
  ...
  # Internal helper used by `EcritsWeb.DocumentLive.dispatch/2`. Submits a
  ...
  `Ecrits.Documents.search/3`; the resulting rows are mapped to the
  ...
  def search_documents(%Context{} = ctx, query) when is_binary(query) do
    ctx
    |> Ecrits.Documents.search(query, 20)
    |> Enum.map(fn doc ->
      %{
        id: doc.id,
        document_id: doc.id,
  ...
  def list_documents(%Context{} = ctx) do
    ctx
    |> Ecrits.Documents.list_all_for_scope()
