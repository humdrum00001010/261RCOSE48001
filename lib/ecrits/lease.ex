defmodule Ecrits.Lease do
  @moduledoc """
  Current live-writer guard. Prevents duplicated `Ecrits.Session` processes
  ...
  check that `Ecrits.Store.append/3` performs inside the commit
  transaction.

  ## Errors

  `assert_current!/2` raises `Ecrits.Lease.FencedOut` when the supplied
  ...
  defmodule FencedOut do
    @moduledoc """
    Raised by `Ecrits.Lease.assert_current!/2` when the supplied fencing
  ...
  @doc """
  Raise `Ecrits.Lease.FencedOut` unless the lease row for `document_id`
  matches the supplied `fencing_token` and is still in its TTL.

  Designed to be called *inside* a `Ecrits.Store.append/3` transaction,
