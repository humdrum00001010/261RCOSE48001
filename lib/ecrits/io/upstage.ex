defmodule Ecrits.IO.Upstage do
  @moduledoc """
  Upstage Document Parse client (multipart upload + element normalization).

  After normalization, `import_upload/3` invokes `Ecrits.IO.IRRefiner.refine/2`
  ...
  @spec import_upload(T.ctx(), T.user_id() | nil, map() | T.upload(), keyword()) ::
          {:ok, Ecrits.Command.t()} | {:error, term()}
  ...
  defp llm_enabled? do
    case Application.get_env(:ecrits, :openai, [])[:api_key] do
  ...
  def parse(file_or_path, opts \\ []) do
    cfg = Application.fetch_env!(:ecrits, :upstage)
  ...
  @doc """
  Lower-level entry called by `Ecrits.IO.parse_source/3` after raw source
  bytes are fetched.
  """
  ...
  defp upload_source(owner_id, artifact_id, info) do
    case File.read(info.path) do
      {:ok, body} ->
        key = source_key(owner_id, artifact_id, info)
        Ecrits.IO.RetiredObjectStore.put(key, body, content_type: info.mime_type)
  ...
  defp fetch_source("r2://" <> rest) do
    ...
    Ecrits.IO.RetiredObjectStore.get(key)
  end
