defmodule Ecrits.IO.OpenAI do
  ...
  @behaviour Ecrits.IO.OpenAI.Behaviour
  ...
  request-scoped `Ecrits.Context` does not have a stored Slack token
  ...
  """
  @spec slack_mcp_tool(Ecrits.Context.t() | nil) :: map() | nil
  def slack_mcp_tool(%Ecrits.Context{} = ctx) do
    case Ecrits.Integrations.Slack.token_for(ctx) do
      {:ok, token} ->
        %{
          type: "mcp",
          server_label: "slack",
  ...
  defp build_request(params, opts) do
    cfg = Application.fetch_env!(:ecrits, :openai)
    ...
    client =
      ...
      |> OpenaiEx.with_finch_name(Ecrits.Finch.OpenAI)
