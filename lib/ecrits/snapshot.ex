defmodule Ecrits.Snapshot do
  @moduledoc """
  Persisted projection row written by `Ecrits.Store.snapshot/2`. The
  `projection` JSONB column holds the full materialized state at a given
  revision, and `r2_key` points at the durable copy in R2.

  Snapshots accelerate `Ecrits.Store.load/1` — instead of folding the
