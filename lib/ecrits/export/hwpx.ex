defmodule Ecrits.Export.HWPX do
  @moduledoc """
  Hand-rolled HWPX (Hangul Word Processor XML / OWPML) writer.

  Takes a `Ecrits.Runtime.State` (or a bare projection map) and emits a
  ...
  * Fields render as inline plain text (the resolved value);
  ...
  Accepts either a `%Ecrits.Runtime.State{}` struct or a bare projection
  ...
  defp version_xml do
    @xml_decl <>
      ...
      ~s( buildNumber="0" xmlVersion="1.4" application="Ecrits.Export.HWPX") <>
      ~s( appVersion="0.1.0"/>)
  end
