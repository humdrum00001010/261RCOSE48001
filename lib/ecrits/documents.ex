defmodule Ecrits.Documents do
  ...
  scope-less helpers called by `Ecrits.Store.append/3` on the hot
  ...
  alias Ecrits.Lease.Record, as: LeaseRecord
  ...
  def complete_write(%Context{} = scope, document_id) do
    with_document_lock(document_id, fn ->
      with {:ok, %Document{} = doc} <- get(scope, document_id),
           ...
           {:ok, head_revision} <- Ecrits.Store.latest_revision(document_id),
  ...
  @doc """
  Set a document's `:title`. Called from `Ecrits.Store.append/3` on the
  ...
  variant used by `Ecrits.Store.append/3`.
  """
  ...
  defp ensure_head_checkpoint(document_id, revision) do
    if Ecrits.RhwpSnapshot.committed_for_revision?(document_id, revision) do
      :ok
    else
