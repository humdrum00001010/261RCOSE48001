defmodule Ecrits.Accounts.UserNotifier do
  @moduledoc """
  ...
  def deliver_update_email_instructions(%User{} = user, url) when is_binary(url) do
    deliver_now(user.email, "이메일 변경 안내 · Ecrits", update_email_body(user, url))
  end
  ...
  def deliver_login_instructions(%User{} = user, url) when is_binary(url) do
    case user do
      %User{confirmed_at: nil} ->
        deliver_now(user.email, "계정 확인 안내 · Ecrits", confirmation_body(user, url))

      _ ->
        deliver_now(user.email, "로그인 안내 · Ecrits", magic_link_body(user, url))
    end
  end
  ...
  defp update_email_body(user, url) do
    intro = "Ecrits에서 이메일 주소 변경을 요청하셨습니다. 아래 버튼을 눌러 새 이메일을 확정해 주세요."
  ...
  defp magic_link_body(user, url) do
    intro = "Ecrits에 로그인하려면 아래 버튼을 눌러주세요. 이 링크는 일회용이며 한 시간 안에 만료됩니다."
    disclaimer = "본인이 요청하지 않았다면 이 메일은 무시하셔도 됩니다."
  ...
  defp confirmation_body(user, url) do
    intro = "Ecrits 가입을 환영합니다. 아래 버튼을 눌러 계정을 확정해 주세요."
      ...
      <table role="presentation" cellspacing="0" cellpadding="0" border="0" align="center" width="100%" style="max-width:520px; margin:0 auto;">
        ...
        <tr>
          <td style="padding:20px 4px 0; font-size:12px; color:#{muted}; line-height:1.6;">
            #{disclaimer}
            <br />
            <span style="color:#{muted}/0.6;">— Ecrits · 비공개 베타</span>
          </td>
        </tr>
      </table>
    </body>
  ...
  defp email_link_frame_attrs do
    if Application.get_env(:ecrits, :dev_routes, false) do
      ""
    else
