defmodule Ecrits.Session do
  ...
  from `Ecrits.Store`. Not truth. See SPEC.md §14, §15.
  ...
  @registry Ecrits.Session.Registry
  ...
  def init(opts) do
    ...
    with {:ok, lease} <- Lease.acquire(document_id, owner_ref),
      ...
      _ = Phoenix.PubSub.subscribe(Ecrits.PubSub, Store.pubsub_topic(document_id))
  ...
  def handle_info(:renew_lease, data) do
    case Lease.renew(data.document_id, data.owner_ref, data.lease.fencing_token) do
      ...
      {:error, reason} ->
        Logger.warning("Ecrits.Session: lease renew failed: #{inspect(reason)}")
        schedule_renew(data.renew_interval_ms)
        {:noreply, data}
    end
  end
