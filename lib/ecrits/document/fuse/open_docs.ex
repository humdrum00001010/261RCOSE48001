defmodule Ecrits.Fuse.OpenDocs do
  @moduledoc """
  Per-workspace ETS state for the doc VFS (`Ecrits.Fuse.DocFs`): write staging,
  the committed-projection cache, the canonical-echo lifecycle, and the
  workspace write-access gate.

  It ALSO holds an open-set registry (`open/3`, `close/2`, `member?/2`,
  `list/1`, `name_for_source/2`, `source_path/2`, the `owner_*` family) that
  nothing populates any more. `doc.open_doc` was its only writer, and the mount
  now derives its listing from the workspace scan intersected with the live
  viewers, so no caller can be told anything. Those functions are kept only
  because step 4 of `docs/plans/2026-07-28-client-owns-document.md` deletes this
  module outright; do not build on them.
  ...
  def owner_agent_id(root, name) do
    case :ets.lookup(@table, {expand(root), name}) do
      [{_key, %{agent_id: agent_id}}] when is_present(agent_id) -> agent_id
      [{_key, %{"agent_id" => agent_id}}] when is_present(agent_id) -> agent_id
      _ -> nil
    end
  rescue
  ...
  def owner_turn_id(root, name) do
    case :ets.lookup(@table, {expand(root), name}) do
      [{_key, %{turn_id: turn_id}}] when is_present(turn_id) -> turn_id
      [{_key, %{"turn_id" => turn_id}}] when is_present(turn_id) -> turn_id
      _ -> nil
    end
  rescue
  ...
  @doc """
  Drop every cached projection under `root`, returning the count cleared.

  `DocFs.projected_bytes/2` serves `committed/2` in preference to re-projecting,
  so the mount keeps handing out the bytes of the last write even after the
  PROJECTION ITSELF would now be built differently — a changed engine, a rebuilt
  bundle, or `Projection.editable_nodes/2` newly deciding an element type does not
  survive the round trip. That produced a mount whose `coverage` said
  `body_paragraphs` while the served buffer still contained `<table>` blocks, so
  every agent edit was rejected as a structural change and the reason was
  invisible from the outside. (The agent under test diagnosed exactly this.)

  Remounting is the explicit "start fresh" signal, so `DocMount.refresh/2` calls
  this. Staged (uncommitted) buffers go too: a stage is a half-finished write
  against the OLD projection shape.
  """
  @spec clear_cached_projections(String.t()) :: non_neg_integer()
  def clear_cached_projections(root) when is_binary(root) do
    root = expand(root)

    [@committed_key, @stage_key]
    |> Enum.flat_map(fn key -> :ets.match_object(@table, {{key, root, :_}, :_}) end)
  ...
  @doc """
  Re-pin every registration owned by `agent_id`/`instance_id` onto `turn_id`.

  INERT since `doc.open_doc` was deleted: registrations were its only source, so
  there is nothing left to re-pin. Kept with its rationale because the fence it
  serves still exists and this module is removed wholesale in step 4 of
  `docs/plans/2026-07-28-client-owns-document.md`.

  A registration recorded the turn that opened the projection, and
  `Projection.write_back/3` fences a write by comparing that recorded turn
  against the session's live `tool_context`. An agent that opened a document in
  one turn and edited it in a LATER turn — without redundantly re-opening — then
  carried a stale `turn_id` and its first write of every subsequent turn was
  rejected with `:turn_invalidated` (surfacing to the agent as an EIO on the
  atomic rename). Measured with `:dbg`:

      WRITE   turn=0de32cc9-…   # pinned when the document was registered
      CONTEXT turn=e2be7b81-…   # the live turn; agent_id/instance_id identical
      -> {error, turn_invalidated}

  The fence's purpose is to reject writes from a CANCELLED or superseded turn,
  not from the same agent legitimately continuing. Advancing the pin at turn
  launch keeps that purpose: only the owning agent+instance is moved forward,
  and a write racing in from a turn that has already been replaced still fails
  the comparison.
  """
  @spec refresh_agent_turn(String.t(), String.t(), String.t()) :: non_neg_integer()
  def refresh_agent_turn(agent_id, instance_id, turn_id)
      when is_present(agent_id) and is_present(instance_id) and is_present(turn_id) do
    @table
    |> :ets.match_object({{:_, :_}, :_})
    |> Enum.reduce(0, fn
      {{_root, _name} = key, metadata}, count when is_map(metadata) ->
        if metadata_value(metadata, :agent_id) == agent_id and
             metadata_value(metadata, :instance_id) == instance_id and
             metadata_value(metadata, :turn_id) != turn_id do
          :ets.insert(@table, {key, Map.put(metadata, :turn_id, turn_id)})
          count + 1
        else
          count
        end
  ...
  def refresh_agent_turn(_agent_id, _instance_id, _turn_id), do: 0
  ...
  clears the matching OpenDocs bookkeeping
  ...
  # Per-workspace write policy for the doc VFS. The mounted projection is read-only
  ...
  @doc "Stage uncommitted projected DocLang bytes for `name` under `root`."
  ...
  @doc "Fetch staged projected DocLang bytes for `name` under `root`."
  ...
  @doc "All staged projected DocLang buffers under `root`."
  ...
  @doc "Remove staged projected DocLang bytes for `name` under `root`."
  ...
  @doc "Cache the exact projected DocLang bytes accepted by a successful VFS write-back."
  ...
  @doc "Fetch the exact projected DocLang bytes from the latest successful VFS write-back."
  ...
  defp maybe_put_agent_id(metadata, agent_id) when is_present(agent_id),
  ...
  defp maybe_put_turn_id(metadata, turn_id) when is_present(turn_id),
    ...
    identity_matches? =
      Enum.all?([:agent_id, :instance_id, :turn_id], fn key ->
        case Keyword.get(filters, key) do
          value when is_present(value) -> Map.get(entry, key) == value
          _unscoped -> true
        end
      end)
  ...
  defp staged_owner_matches?(identity, filters) when is_map(identity) do
    Enum.all?(filters, fn {key, value} ->
      not is_present(value) or Map.get(identity, key) == value
    end)
  end
