defmodule Ecrits.Fuse.DocFs do
  ...
  (`sed -i`, a formatter) can rewrite. Two write patterns are supported:
    ...
    * **in-place** — `open`/`truncate`/`write` the projected file, commit as soon
      as the buffered projected XML becomes a complete DocLang document, with
      `release` as the final safety commit;
    * **atomic temp+rename** (what most linters do) — `create` a transient file
      ...
      projected `<doc>.doclang.xml`; the `rename` commits the temp's bytes via
      `write_back`.

  ## Why mid-write garbage is safe

  Commit is attempted on EVERY write and the incomplete ones are staged, so the
  format must make "complete" decidable from the buffer alone. It does: an XML
  document has exactly one root, and `</doclang>` is therefore its LAST token.
  `Doclang.Xml` never auto-closes at end of input — an element whose content
  runs out fails `expect_close` — so any prefix of a projected document is a
  parse error, never a shorter valid document. Out-of-order FSKit delivery is
  covered the same way: `splice/3` NUL-pads the gap, and a NUL is neither XML
  whitespace nor a legal name/content start, so the prolog scan rejects it.
  (This is the same property the previous nested-JSONL mount relied on, where
  the trailing `]` played the role of `</doclang>`.)

  Incomplete buffers may therefore be staged so editor/linter recovery can
  finish a later whole-file rewrite. Complete documents with the wrong shape
  (a non-`doclang` root, or a block tree that cannot be aligned with the live
  document) fail instead of replacing the mounted truth. Only a buffer that was
  actually WRITTEN commits: a read-only `cat`/`open` seeds a buffer but is never
  dirty, so it never writes back (critical — otherwise a stale read seed would
  revert a concurrent rename). A write is gated on the workspace access policy
        ...
        cond do
          ...
          true ->
            ...
            case write_back_unless_accepted(
              ...
              {:ok, _info} ->
                settlement =
                  accepted_projection_settlement(root, source, bytes, identity)
  ...
  # projected `<doc>.doclang.xml`. Commit the temp's bytes to the document and drop both
  ...
  # `report.hwp.doclang.xml.swp` -> `report.hwp.doclang.xml`: an editor's temp
  # sibling of a projected file previews against that file's source.
  defp preview_target_name(name) when is_binary(name) do
    suffix = Projection.projected_suffix()

    case String.split(name, suffix, parts: 2) do
      [base, rest] when rest != "" ->
        if String.starts_with?(rest, "."), do: {:ok, base <> suffix}, else: :error
  ...
  defp cache_accepted_projection(root, source_name, source, bytes, identity) do
    case accepted_projection_settlement(root, source, bytes, identity) do
  ...
  defp accepted_projection_settlement(root, source, bytes, identity) do
    metadata = Map.put(identity || %{}, :source_path, source)

    case Projection.project_file(source, root: root) do
      {:ok, canonical_bytes} ->
        {:accept_projection, bytes, canonical_bytes, metadata}
  ...
  defp safe_resolve_in_flight_canonical(root, entry, opts) do
    project_fun =
      Keyword.get(opts, :project_fun, fn source -> Projection.project_file(source, root: root) end)
  ...
  # `{:malformed_xml, _}` is the mid-write case: the buffer is not yet a complete
  # DocLang document (the root `</doclang>` has not arrived, or an out-of-order
  # chunk left a NUL gap). Stage it and let a later chunk finish the rewrite.
  defp retryable_commit_error?({:malformed_xml, _detail}), do: true
  ...
  # correctable DocLang edit from a transport/runtime failure on both release
  # and temp+rename paths.
  defp write_errno(:stale_projection_replay), do: :einval
  defp write_errno({:structural_change, _detail}), do: :einval
  defp write_errno({:malformed_xml, _detail}), do: :einval
  defp write_errno({:unexpected_root, _name}), do: :einval
  defp write_errno({:unsupported_edit, _what}), do: :einval
  ...
  # failures are user-correctable DocLang input and must never unlock `doc.edit`.
  ...
  defp terminal_rejectable_staged_error?({:malformed_xml, _detail}), do: true
  ...
  # A COMPLETE XML document that is not a `<doclang>` root is not a projection
  # at all — no later chunk can turn it into one, so it must not linger as a
  # staged candidate the way a truncated buffer legitimately does.
  defp discardable_release_stage_error?({:unexpected_root, _name}), do: true
