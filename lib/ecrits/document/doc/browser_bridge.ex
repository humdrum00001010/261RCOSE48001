defmodule Ecrits.Doc.BrowserBridge do
  ...
  The caller remains responsible for finding the viewer that holds the document
  via `Ecrits.Workspace.Session.viewer/2`. This module only transports a request to an
  ...
  def call(lv, verb, payload, opts \\ [])
    ...
    request =
      if is_present(expected_document_id) do
