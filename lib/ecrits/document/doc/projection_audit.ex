defmodule Ecrits.Doc.ProjectionAudit do
  ...
  Input is the mounted projection's bytes — **DocLang v0.6 XML** since
  2026-07-26 — or an already-decoded payload tree. `decode_projection/1` lowers
  DocLang blocks into the payload-node vocabulary the checks below are written
  ...
  def validate_standard_contract_demo(projection, document_path, image_path, opts)
    ...
    # `native_marker_geometry/2` can only report why it is unavailable: the
    # native engine it measured with left with `:ehwp` (2026-07-26). The audit
    # therefore always runs in allow-missing mode and records the reason; the
    # {:ok, geometry} arm was unreachable.
    {:error, reason} = native_marker_geometry(document_path, picture_requirement)

    geometry_opts = [allow_missing_native_marker_geometry: true]
  ...
  # The mount serves DocLang v0.6 XML (`Ecrits.Doc.Projection`). The audit's
  # whole vocabulary — paragraph text, `row`/`col` table cells, picture payloads,
  # payload GROUPS scoped by `before:`/`after:` anchors — predates that and is
  # phrased in the engine's payload nodes, so parsed DocLang blocks are lowered
  # back into that shape here rather than restating 2000 lines of index-free
  # requirements in DocLang terms.
  #
  # One top-level DocLang block becomes one payload group, which is the same
  # granularity the JSONL projection's paragraph groups had. Callers may still
  # pass an already-decoded payload tree (the requirement fixtures do).
  ...
  defp decode_projection(projection) when is_binary(projection) do
    case Doclang.parse_blocks(projection) do
      {:ok, blocks} -> {:ok, Enum.map(blocks, &payload_group/1)}
  ...
  defp payload_group(%{"type" => "table"} = block) do
    ...
    [%{"type" => "table"}] ++
      Enum.flat_map(cells, fn cell ->
        [
          %{
            "type" => "cell",
            "row" => Map.get(cell, "row", 0),
            "col" => Map.get(cell, "col", 0)
          }
          | Enum.flat_map(Map.get(cell, "blocks", []), &payload_group/1)
        ]
      end)
  end

  defp payload_group(%{"type" => "list", "items" => items}) when is_list(items) do
    Enum.flat_map(items, fn item ->
      Enum.flat_map(Map.get(item, "blocks", []), &payload_group/1)
    end)
  end

  defp payload_group(%{"type" => "picture"} = block) do
    [maybe_put_payload(%{"type" => "picture"}, "src", Map.get(block, "src"))]
  end

  defp payload_group(%{"type" => "page_break"}), do: [%{"type" => "page_break"}]

  defp payload_group(%{"type" => type} = block)
       when type in ["text", "heading", "formula"] do
    [%{"type" => "paragraph", "text" => Doclang.text(block)}]
  end

  # Containers (`footnote`, `page_header`/`page_footer`, `group`, and any
  # forward-compatible element this reader does not model) contribute their own
  # node plus their children, flattened into the same group.
  defp payload_group(%{"type" => type} = block) when is_binary(type) do
    [%{"type" => type} | Enum.flat_map(Map.get(block, "blocks", []), &payload_group/1)]
  end

  defp payload_group(_block), do: []
  ...
  # Marker geometry came from the headless `ehwp` NIF (Ehwp.open/find/query),
  # removed 2026-07-26 with the engine deps. The caller
  # (`validate_standard_contract_demo/4`) already has a first-class degradation
  # path for this — it sets `allow_missing_native_marker_geometry: true` and
  # records a `:native_marker_geometry_unavailable` issue — so report unavailable
  # rather than pretending. Restore via the browser engine once DocLang lands.
  # See docs/plans/2026-07-26-doclang-engine-migration.md.
  defp native_marker_geometry(document_path, _requirement) do
    if File.regular?(document_path) do
      {:error, :native_engine_removed}
    else
