defmodule Ecrits.Doc.MCPServer do
  ...
  *discovers* and *calls* these tools itself; ex_mcp routes each call here, we
  ...
  # The supplied `mounted_at` is checked against the LIVE mount, not against a
  # path a tool once returned: it must sit inside this workspace's mount point
  # and name a document `Ecrits.Fuse.DocFs` is currently serving. That is the
  # same derived predicate `readdir`/`getattr` answer, so the fallback can only
  # target a file the agent could actually have read and written.
  #
  # `File.exists?` on the mount is deliberately NOT part of it: an in-BEAM
  # `File.*` against an exfuse mount can deadlock the global `:file_server`, and
  # `served?/2` already proves the SOURCE document exists (via `:prim_file`).
  defp verify_vfs_fallback_mount(
    ...
    with root when is_present(root) <- Map.get(ctx, :session_path),
         ...
         true <- Ecrits.Fuse.DocFs.served?(root, name) do
      :ok
    else
  ...
  defp verify_vfs_picture_source(ctx, %{"op" => %{"src" => src}}) when is_binary(src) do
    with root when is_present(root) <- Map.get(ctx, :session_path),
         ...
         {:ok, normalized} <- Workspace.relative_path(workspace, src),
         {:ok, resolved_src} <- Workspace.native_path(workspace, normalized),
         true <- resolved_src == Path.expand(src),
         {:ok, %Exfuse.Fs.Stat{type: :file}} <- Workspace.stat(workspace, normalized) do
      :ok
    else
      _ ->
        {:error,
  ...
  defp vfs_fallback_mount_required_message do
    %{
      ...
      "tool" => "doc.edit",
      "message" =>
        "doc.edit fallback requires mounted_at to be the projection file you committed, inside this workspace's live mount. If that path is missing, not a served projection, or outside the mount, report the VFS blocker instead of bypassing it.",
      "required_fallback" => %{"mounted_at" => "<workspace>/.ecrits/<document>.doclang.xml"}
    }
  end
  ...
  # `primary_committed?` no longer compares against a per-turn baseline revision.
  # The baseline was captured by `doc.open_doc`, which ran BEFORE the agent's
  # edit; the only capture point left is the first `doc.*` call of the turn,
  # which now happens AFTER it — so the comparison could only ever answer false
  # and would close the native fallback permanently. What survives still requires
  # a committed projection, no recorded write failure and nothing staged, and
  # `check_find_pattern_addressable/3` separately requires the requested
  # paragraph to exist verbatim in the committed bytes — which an uncommitted
  # edit cannot satisfy.
  defp vfs_sequence_evidence(ctx, %{phase: :acp_primary}, "doc.find", args) do
    current = committed_projection(ctx)
    pattern = Map.get(args, "pattern")

    %{
      committed_projection?: is_binary(current),
      primary_committed?:
        is_binary(current) and no_vfs_write_failure?(ctx) and no_vfs_stage?(ctx),
      exact_count:
  ...
  # Which document this turn is about, DERIVED. `doc.open_doc` used to record it
  # from its own `{path: "current"}` resolution; the same two facts — the
  # workspace root and the tab-bound relative path — answer it without a tool.
  defp turn_mount_name(ctx) do
    with root when is_present(root) <- Map.get(ctx, :session_path),
         relative when is_present(relative) <- Map.get(ctx, :document_path) do
      root = Ecrits.Fuse.DocMount.canonical_root(root)
      {:ok, root, Ecrits.Fuse.DocMount.mount_name(Path.relative_to(relative, root))}
    else
      _ -> :error
    end
  end

  defp committed_projection(ctx) do
    with {:ok, root, mount_name} <- turn_mount_name(ctx),
         {:ok, bytes} <- Ecrits.Fuse.OpenDocs.committed(root, mount_name) do
      bytes
    else
      _ -> nil
    end
  ...
  defp no_vfs_write_failure?(ctx) do
    with {:ok, root, mount_name} <- turn_mount_name(ctx),
         {:ok, _reason} <- Ecrits.Fuse.OpenDocs.write_failure(root, mount_name) do
      false
    else
      _ -> true
    end
  ...
  defp no_vfs_stage?(ctx) do
    with {:ok, root, mount_name} <- turn_mount_name(ctx),
         {:ok, _bytes, _reason} <- Ecrits.Fuse.OpenDocs.staged(root, mount_name) do
      false
    else
      _ -> true
    end
  ...
  # bound to) and dispatch the tool there, so `doc.context` returns THIS agent's
  # active doc and `doc.open`/`doc.edit` honour per-agent ownership.
  #
  # An absent agent id (legacy bare mount, or a non-agent caller in a test) gets
  # an EMPTY context: no `:session_path`, therefore no viewer registry, therefore
  # no routable document. An agent id that does NOT resolve (dead/unknown) is
  # rejected so a tool never silently runs against the wrong context.
  # `document_path` is the UI-selected workspace path and is surfaced by
  # `doc.context.current_document` so agents do not need prompt-embedded
  # document names.
  defp resolve_tool_context(nil), do: {:ok, %{}}
      ...
      {:ok, pid} ->
        ...
        {:ok,
         %{
           ...
           # and the human-viewer registry that decides where a document lives.
