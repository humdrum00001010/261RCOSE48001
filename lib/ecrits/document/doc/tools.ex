defmodule Ecrits.Doc.Tools do
  ...
  @tools [
    ...
    %{
      ...
      "name" => "open_doc",
      "description" =>
        "Open a workspace document on the primary editing surface. `path` is " <>
          "either `current` (the document bound to the open editor tab) or the " <>
    ...
    %{
      ...
      "description" =>
        "Remove a document from the doc VFS mount (reverse of doc.open_doc): the " <>
          "projected <name>.doclang.xml disappears from <workspace>/.ecrits/. " <>
    ...
    with {:ok, path} <- require_string(args, "path"),
      ...
      {:ok,
       %{
         ...
         "projection_coverage" => open_projection_coverage(root, abs, mounted_at),
  ...
  defp cache_open_projection(root, name, abs, mounted_at, nil) when is_binary(mounted_at) do
    case OpenDocs.committed(root, name) do
      ...
      :error ->
        case Ecrits.Doc.Projection.project_file(abs, root: root) do
          {:ok, bytes} ->
            OpenDocs.cache_committed(root, name, bytes)
            {mounted_at, nil}

          # A no-engine failure is not a mount failure and must not read like
          # one: the mount is fine, the DOCUMENT has no live model to project.
          # Report the projection's own explanation verbatim so the agent can
          # act on it (open the document, retry) instead of guessing.
          {:error, {:no_engine, info}} ->
            {nil, "no_engine: " <> Map.get(info, :hint, inspect(info))}
  ...
  # What the live arm could actually enumerate. Best-effort and never fatal: a
  # missing coverage report must not fail an otherwise-good open, but a PARTIAL
  # one has to reach the agent — that is the difference between "the tables are
  # not in this file" and "this document has no tables".
  defp open_projection_coverage(_root, _abs, nil), do: nil

  defp open_projection_coverage(root, abs, _mounted_at) do
    case Ecrits.Doc.Projection.coverage(abs, root: root) do
    ...
    case get(args, ["ops"]) do
      ...
      _ ->
        with {:ok, op} <- require_map(args, "op") do
          route_doc(ctx, args,
            # Viewed-HWP authority is the browser engine model: deliver the structural
            # edit to the owning LiveView -> the canvas hook applies it to the browser
            # engine's document. We do NOT touch the server NIF
