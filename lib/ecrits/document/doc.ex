defmodule Ecrits.Doc do
  ...
  NO IMPLEMENTATIONS since 2026-07-29. `Ecrits.Doc.Rhwp` (the `ehwp` NIF) and
  `Ecrits.Doc.Office` (the `libreofficex` NIF) went with the engine deps on
  2026-07-26, and the last thing that could have called a backend —
  `Ecrits.Doc.Editor`, started only by `Ecrits.Doc.Pool` — went with the Pool.
  What is still LIVE here is the type vocabulary (`kind`, `ref`, `applied`),
  which the projection and tool layers spec against; the callbacks below are the
  shape a server engine would have to take if one returns.
  ...
  engine-level effect, never a document ordering token.
  """
  ...
  # `backend_for/1` DELETED 2026-07-28: it mapped a document kind to a server-NIF
  # backend module and had answered `nil` for every kind since
  # `:ehwp`/`:libreofficex` were removed. Its only caller was the `{:server,
  # editor}` arm of `Projection.engine/4`, which could not be reached either —
  # `Ecrits.Doc.Pool` refused to open any document, so no Editor was ever
  # started. Pool and Editor followed on 2026-07-29. Engine authority is the
  # browser, reached through `Ecrits.Workspace.Session.viewer/2`.
  #
