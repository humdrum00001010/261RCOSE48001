defmodule Ecrits.WorkspaceHandoff do
  ...
  def put_workspace_path(live_session_id, path)
      when is_present(live_session_id) and is_present(path) do
  ...
  def fetch_workspace_path(live_session_id)
      when is_present(live_session_id) do
  ...
  def put_chat_rail_state(workspace_path, rail_state)
      when is_present(workspace_path) and is_map(rail_state) do
  ...
  def put_agent_state(workspace_path, agent_id, agent_state)
      when is_present(workspace_path) and is_present(agent_id) and is_map(agent_state) do
  ...
  def reset_agent_state(workspace_path, agent_id)
      when is_binary(workspace_path) and is_present(agent_id) do
  ...
  def delete_chat_rail(workspace_path, rail_key)
      when is_present(workspace_path) and is_present(rail_key) do
  ...
  def fetch_chat_rail_state(workspace_path)
      when is_present(workspace_path) do
  ...
  defp accept_agent_instance(
       ...
       when is_present(current) and is_present(incoming) and
              current != incoming,
  ...
  defp default_store_path do
    if Application.get_env(:ecrits, :env) == :test do
      # pid-scoped so concurrent runs do not share one store.
      Path.join(System.tmp_dir!(), "handoff-#{System.pid()}.json")
    else
