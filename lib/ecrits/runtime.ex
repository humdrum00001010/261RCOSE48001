defmodule Ecrits.Runtime do
  @moduledoc """
  Routes `Ecrits.Command`s into the correct execution path
  ...
  ## Routing table
      ...
      :chat_message                → Ecrits.Agent.Document.start
  """
  ...
  alias Ecrits.Runtime, as: Self
  alias Ecrits.Session
  alias Ecrits.Store
  ...
  @spec load(T.ctx(), T.document_id()) :: T.result(Ecrits.Runtime.State.t())
  ...
  def apply(ctx, %Command{kind: :chat_message} = action) do
    with {:ok, _thread, persisted_action, _message} <-
      ...
      Ecrits.Agent.Document.start(ctx, persisted_action)
    end
  end
  ...
  defp create_document_directly(ctx, %Command{} = action) do
    ...
    with {:ok, %Document{} = doc} <- get_or_create_document(ctx, attrs) do
      ...
      empty_state = %Ecrits.Runtime.State{document_id: document_id, revision: 0}

      with {:ok, %Ecrits.ChangeInput{} = input} <-
             Ecrits.Session.Reducer.compile(action, empty_state),
           {:ok, _} <- Ecrits.Session.Reducer.validate(input, empty_state),
           {:ok, preimage} <- Ecrits.Session.Reducer.preimage(input, empty_state),
           {:ok, inverse_ops} <- Ecrits.Session.Reducer.inverse(input, preimage),
           {:ok, affected_refs} <- Ecrits.Session.Reducer.affected_refs(input, empty_state),
           input = %Ecrits.ChangeInput{
             input
             | preimage: preimage,
               inverse_ops: inverse_ops,
               affected_refs: affected_refs
           },
           {:ok, change} <- Ecrits.Session.Reducer.build_change(action, input, empty_state) do
        ...
        with {:ok, lease} <- Ecrits.Lease.acquire(document_id, "system:create"),
             {:ok, persisted} <- Store.append(document_id, change, lease.fencing_token) do
          _ = Ecrits.Lease.release(document_id, "system:create", lease.fencing_token)
          {:ok, persisted}
        end
      end
    end
