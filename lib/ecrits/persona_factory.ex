if Application.compile_env(:ecrits, :test_auth, false) do
  defmodule Ecrits.PersonaFactory do
    @moduledoc """
    ...
    Gated behind `Application.compile_env(:ecrits, :test_auth, false)` so
    ...
    `true`, which makes the module compilable and lets:
      ...
      * `EcritsWeb.TestAuthController` (a `/test/personas/:p/sign_in` route)
        consume the factory to mint a real session cookie for Playwright.
      * `Ecrits.PersonaFactoryTest` / Wallaby `EcritsWeb.FeatureCase`
    ...
    alias Ecrits.{Accounts, Context, Repo}
    ...
    if Code.ensure_loaded?(Wallaby.Browser) do
      ...
      @doc """
      Returns the `%Ecrits.Context{}` for the persona signed in via
