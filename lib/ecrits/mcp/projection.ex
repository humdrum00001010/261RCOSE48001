defmodule Ecrits.MCP.Projection do
  @moduledoc """
  Adapter from `Ecrits.Runtime.State` projection (node-graph) to the flat
  ...
  local-first HWP/HWPX snapshots are stored under `.ecrits` by
  `Ecrits.Local.Document`.
  ...
  def to_agent_ir(%State{} = state) do
    case latest_rhwp_snapshot(state) do
      %Ecrits.RhwpSnapshot.Record{} = snap ->
  ...
  def current_snapshot_revision(%State{} = state) do
    case latest_rhwp_snapshot(state) do
      %Ecrits.RhwpSnapshot.Record{revision: revision} when is_integer(revision) -> revision
      _ -> nil
    end
  end
  ...
  defp snapshot_raw_ir(%Ecrits.RhwpSnapshot.Record{} = snap) do
  ...
  defp validate_same_revision_text_basis(
         %Ecrits.RhwpSnapshot.Record{document_id: document_id, revision: revision} = snap,
         %State{} = state,
         raw_ir
       )
  ...
  defp prior_rhwp_snapshots(%Ecrits.RhwpSnapshot.Record{
       ...
       when is_binary(document_id) and is_integer(revision) do
    Repo.all(
      from s in Ecrits.RhwpSnapshot.Record,
  ...
  defp pending_ops_disjoint_from_unmaterialized_text?(%State{} = state, pending_ops) do
    with %Ecrits.RhwpSnapshot.Record{} = snap <- latest_rhwp_snapshot(state),
         %Ecrits.RhwpSnapshot.Record{} = previous <- previous_rhwp_snapshot(snap) do
      changed_keys =
        snap.document_id
  ...
  defp previous_rhwp_snapshot(%Ecrits.RhwpSnapshot.Record{
         document_id: document_id,
         revision: revision,
         format: format
       })
  ...
  defp latest_rhwp_snapshot(%State{} = state) do
    Ecrits.RhwpSnapshot.latest_for_document(state.document_id, snapshot_format_for_state(state))
  end
  ...
  defp snapshot_format_for_state(%State{projection: projection}) do
    with type_key when is_binary(type_key) and type_key != "" <-
         ...
         {:ok, spec} <- Ecrits.ContractTypes.get(type_key) do
  ...
  defp from_db_projection(
         %Ecrits.RhwpSnapshot.Record{projection: %{} = snap},
         %State{} = state
       )
