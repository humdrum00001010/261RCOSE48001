defmodule Ecrits.DocumentCanvasState do
  ...
  defp spec_path(%{template_hwp_path: path}) when is_present(path), do: path
  defp spec_path(%{template_hwpx_path: path}) when is_present(path), do: path
  defp spec_path(_spec), do: nil

  defp spec_name(%{name: name}, _path) when is_present(name), do: name
