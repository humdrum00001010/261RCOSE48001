defmodule Ecrits.DocumentSearch do
  ...
  # `enabled` gates the host ⌘F handler. Markdown has no find surface at all;
  # hwp/hwpx render inside the rhwp-studio iframe, which owns its own find UI
  # and never hears the host's `ecrits:document-search-command` — so the host
  # find bar is not rendered for them and ⌘F must not try to open it.
  @host_search_disabled_formats ["md", "markdown", "hwp", "hwpx"]

  def encode(%__MODULE__{} = search, document) do
    Jason.encode!(%{
      open: search.open?,
      enabled: is_map(document) and document.format not in @host_search_disabled_formats,
