defmodule Ecrits.Local.Agent.ToolRegistry do
  @moduledoc """
  ...
  alias Ecrits.Local.Agent.DocumentTools
  ...
  @tools [
    ...
    %{
      ...
      "name" => "find",
      "description" =>
        "doc.find(pattern, at?, size?, case_sensitive?): native literal search over active local RHWP document text.",
      "risk" => "read",
      "inputSchema" => %{
        "type" => "object",
        "additionalProperties" => false,
        "properties" => %{
          "pattern" => %{
            "type" => "string",
            "minLength" => 1,
            "description" => "Literal text to search for in the active document."
          },
          "at" => %{
            "type" => "integer",
            "minimum" => 0,
          ...
          "case_sensitive" => %{
            ...
            "default" => false,
            "description" =>
              "When false, search is case-insensitive when supported by the runtime."
          }
        },
        "required" => ["pattern"]
      },
    ...
    %{
      ...
      "name" => "write",
      "description" =>
        "doc.write requests a native document text replacement. Native write persistence is not available yet, so unsupported writes fail explicitly.",
      "risk" => "write",
      "inputSchema" => %{
        "type" => "object",
        "additionalProperties" => false,
        "properties" => %{
          "query" => %{
            "type" => "string",
            "minLength" => 1,
            "description" => "Existing text to replace once."
          },
          "replacement" => %{
            "type" => "string",
