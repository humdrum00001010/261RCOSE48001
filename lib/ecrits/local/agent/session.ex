defmodule Ecrits.Local.Agent.Session do
  @moduledoc """
  ...
  @registry Ecrits.Local.Agent.SessionRegistry
  ...
  def init(opts) do
    ...
    {:ok,
     %{
       ...
       adapter: Keyword.get(opts, :adapter, Ecrits.Local.Agent.Adapters.Unavailable),
  ...
  defp handle_adapter_event(state, turn_id, %{type: type, title: title})
       when type in [:title_generated, :title_updated, :thread_title] and is_binary(title) do
    emit(state, %{type: type, turn_id: turn_id, title: title})
  end

  defp handle_adapter_event(state, turn_id, %{"type" => type, "title" => title})
       when type in ["title_generated", "title_updated", "thread_title"] and is_binary(title) do
    emit(state, %{type: String.to_existing_atom(type), turn_id: turn_id, title: title})
  end
