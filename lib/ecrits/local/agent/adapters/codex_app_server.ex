defmodule Ecrits.Local.Agent.Adapters.CodexAppServer do
  @moduledoc """
  ...
  return explicit errors to `Ecrits.Local.Agent.Session`.
  """
  ...
  defp provider_tool_started(%{"type" => "mcpToolCall"} = item) do
    %{
      type: :tool_call_started,
      tool_call_id: provider_tool_call_id(item),
      name: provider_tool_name(item),
      arguments: provider_tool_arguments(item)
    }
  end

  defp provider_tool_started(%{"type" => "dynamicToolCall"} = item) do
    %{
      type: :tool_call_started,
      tool_call_id: provider_tool_call_id(item),
      name: provider_tool_name(item),
  ...
  defp provider_tool_completed(%{"type" => "mcpToolCall"} = item) do
    if error = provider_tool_error(item) do
      %{
      ...
      %{
        ...
        tool_call_id: provider_tool_call_id(item),
        name: provider_tool_name(item),
        result: provider_tool_result(item)
      }
    end
  end

  defp provider_tool_completed(%{"type" => "dynamicToolCall"} = item) do
    if error = provider_tool_error(item) do
      %{
        type: :tool_call_failed,
        tool_call_id: provider_tool_call_id(item),
  ...
  defp provider_tool_call_id(item) do
    Map.get(item, "id") ||
      Map.get(item, "tool_call_id") ||
      Map.get(item, "toolCallId") ||
      Map.get(item, "callId") ||
      Map.get(item, "itemId") ||
      Ecto.UUID.generate()
  end

  defp provider_tool_name(item) do
    join_tool_name(
      Map.get(item, "server") || Map.get(item, "namespace"),
      Map.get(item, "tool") || Map.get(item, "name") || Map.get(item, "toolName")
    )
  end

  defp provider_tool_arguments(item), do: Map.get(item, "arguments") || %{}
  defp provider_tool_error(item), do: Map.get(item, "error") || Map.get(item, "failure")

  defp provider_tool_result(item) do
    Map.get(item, "result") ||
      Map.get(item, "output") ||
  ...
  defp initialize_params do
    %{
      clientInfo: %{
        name: "ecrits-local-agent",
        title: nil,
        version: Application.spec(:ecrits, :vsn) |> to_string()
      },
    ...
    %{
      ...
      developerInstructions:
        Keyword.get(
          ...
          "Answer the user's local ecrits workspace request. Do not edit files unless explicitly asked."
        )
    }
