defmodule Ecrits.Session.Reducer do
  @moduledoc """
  Pure mechanics for turning a `Ecrits.Command` into a durable
  `Ecrits.Change`. See SPEC.md §13.
  ...
  helpers". Previously lived under `Ecrits.Engine` (pre-v0.5).
  ...
  * `:chat_message` — `Ecrits.Agent`.
  * source/import/export/revoke/conversion/duplicate commands — pruned
    DB-backed surfaces.
  """

  alias Ecrits.{Change, ChangeInput, Command, MarkInput, Operation, Runtime, Types}
  ...
  ## Examples
      ...
      iex> state = %Ecrits.Runtime.State{revision: 0}
      iex> command = %Ecrits.Command{
      ...
      ...> }
      iex> {:ok, input} = Ecrits.Session.Reducer.compile(command, state)
  ...
  def compile(%Command{kind: kind}, _state) when kind in @unsupported_kinds do
    raise ArgumentError,
          "Ecrits.Session.Reducer.compile/2 does not handle Command.kind=#{inspect(kind)} — " <>
  ...
  `Ecrits.Session.Revocation` (SPEC.md §17).
  ...
  targets. Used by `Ecrits.Session.Revocation` overlap detection (SPEC.md
  §17).
  """
  ...
  @doc """
  Construct a durable `Ecrits.Change` struct from `(command, change_input,
  ...
  defp build_ops_and_marks(%Command{kind: kind}, _state) do
    raise ArgumentError,
          "Ecrits.Session.Reducer: unhandled Command.kind=#{inspect(kind)} in build_ops_and_marks/2"
  end
