defmodule Mix.Tasks.Ecrits.Imagegen do
  @moduledoc """
  ...
  ## Usage
      ...
      mix ecrits.imagegen                          # generate any missing outputs
      mix ecrits.imagegen --force                  # regenerate everything
      mix ecrits.imagegen --manifest path/to.exs   # use a non-default manifest
      mix ecrits.imagegen --only hero,feature-citation  # only these slugs
  ...
  def run(args) do
    ...
    api_key =
      case System.fetch_env("OPENAI_API_KEY") do
        ...
        :error ->
          Mix.raise(
            "OPENAI_API_KEY is not set. Source your .env or export it, then re-run mix ecrits.imagegen."
          )
      end
  ...
  defp generate!(entry, api_key) do
    ...
    # `Application.put_env(:ecrits, :imagegen_req_options, ...)` so the
    # task can be exercised without real OpenAI traffic.
    extra_opts = Application.get_env(:ecrits, :imagegen_req_options, [])
