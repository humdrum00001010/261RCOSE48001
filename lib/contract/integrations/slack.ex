defmodule Contract.Integrations.Slack do
  ...
  def complete_oauth(%Scope{} = ctx, code, redirect_uri, opts \\ []) do
    case Application.get_env(:contract, :slack_oauth_provider) do
  ...
  def token_for(ctx) do
    case Application.get_env(:contract, :slack_token_provider) do
  ...
  def connection_info(ctx) do
    case Application.get_env(:contract, :slack_connection_provider) do
      fun when is_function(fun, 1) -> fun.(ctx)
      _ -> {:error, :not_connected}
    end
  end
