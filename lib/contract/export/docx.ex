defmodule Contract.Export.DOCX do
  @moduledoc """
  DOCX renderer.

  Pipeline: project → HTML (via `Contract.Export.HTML.render/2`) →
  ...
  The pandoc binary path is resolved from (in order):
    ...
    2. `Application.get_env(:contract, :pandoc_path)`.
  ...
  defp find_pandoc(opts) do
    candidate =
      Keyword.get(opts, :pandoc_path) ||
        Application.get_env(:contract, :pandoc_path) ||
