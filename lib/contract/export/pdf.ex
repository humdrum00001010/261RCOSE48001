defmodule Contract.Export.PDF do
  @moduledoc """
  PDF renderer.

  Pipeline: project → HTML (via `Contract.Export.HTML.render/2`) → write
  ...
  The Chromium binary path is resolved from (in order):
    ...
    2. `Application.get_env(:contract, :chromium_path)` (set in
  ...
  `Contract.Export.HTML.render/2` for the deterministic projection
  ...
  defp find_chromium(opts) do
    candidate =
      Keyword.get(opts, :chromium_path) ||
        Application.get_env(:contract, :chromium_path) ||
