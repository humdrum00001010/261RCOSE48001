defmodule ContractWeb.Live.Studio.Components.GrillRail do
  @moduledoc """
  ...
  ## Assigns
    ...
    * `:grill_marks` — list of mark records (structs or maps) the parent
      ...
      treats both `%Contract.MarkInput{}` structs and plain maps the same
    ...
    * `:current_scope` — `%Contract.Context{}`. The component reads
      `:perms` to gate the answer flow:
        ...
        - All other personas (`:lawyer`, `:paralegal`, `:admin`) get the
          full submit input.
    * `:studio_state` — current `%Contract.Studio.State{}`; used so the
  ...
  @doc """
  Resolves the persona-level render mode from a `%Contract.Context{}`.
